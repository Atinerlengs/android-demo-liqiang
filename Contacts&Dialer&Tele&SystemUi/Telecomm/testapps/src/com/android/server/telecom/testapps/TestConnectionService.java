...
public class TestConnectionService extends ConnectionService {
    ...
    final class TestConnection extends Connection {
        ...
        // M: Receive downgrade event and handle it. @{
        private BroadcastReceiver mRttDowngradeReceiver = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                sendRttInitiationFailure(
                        MtkConnection.MtkRttModifyStatus.SESSION_DOWNGRADED_BY_REMOTE);
            }
        };
        // @}
        ...
        TestConnection(boolean isIncoming, ConnectionRequest request) {
            ...
            // M: Register receiver to receive downgrade event. @{
            LocalBroadcastManager.getInstance(getApplicationContext()).registerReceiver(
                    mRttDowngradeReceiver,
                    new IntentFilter(TestCallActivity.ACTION_REMOTE_RTT_DOWNGRADE));
            // @}
        }
