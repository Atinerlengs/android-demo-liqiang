...
public class CallServiceNotifier {
    ...
    // M: Test RTT incoming/upgrade/downgrade. @{
    private static final int RTT_CALL_NOTIFICATION_ID = 3;
    // @}
    ...
    public void updateNotification(Context context) {
        ...
        // M: Add notification to test RTT. @{
        getNotificationManager(context).notify(
                RTT_CALL_NOTIFICATION_ID, getRttNotification(context));
        // @}
    }
    ...
    public void cancelNotifications(Context context) {
        ...
        // M: Add notification to test RTT. @{
        getNotificationManager(context).cancel(RTT_CALL_NOTIFICATION_ID);
        // @}
    }
    ...
    public void registerPhoneAccount(Context context) {
        ...
        // M: Remove it to make incoming call success, this account is for remote
        // connection service, it's useless currently. @{
        /*telecomManager.registerPhoneAccount(PhoneAccount.builder(
                        new PhoneAccountHandle(
    ...
    /**
     * M: Creates a notification object out of the current calls state.
     */
    private Notification getRttNotification(Context context) {
        final Notification.Builder builder = new Notification.Builder(context);
        builder.setOngoing(true);
        builder.setPriority(Notification.PRIORITY_HIGH);
        builder.setSmallIcon(android.R.drawable.stat_sys_phone_call);
        builder.setContentText("Test RTT calls");
        builder.setContentTitle("RTT incoming, Upgrade, Downgrade");

        addRttIncomingAction(builder, context);
        addRttUpgradeAction(builder, context);
        addRttDowngradeAction(builder, context);
    ...
    /**
     * M: Creates the intent to start an incoming RTT call
     */
    private PendingIntent createRttIncomingIntent(Context context) {
        final Intent intent = new Intent(CallNotificationReceiver.ACTION_RTT_CALL,
                null, context, CallNotificationReceiver.class);
        return PendingIntent.getBroadcast(context, 0, intent, 0);
    }

    /**
     * M: Creates the intent to start a RTT upgrade request
     */
    private PendingIntent createRttUpgradeIntent(Context context) {
        final Intent intent = new Intent(TestCallActivity.ACTION_REMOTE_RTT_UPGRADE,
                null, context, TestCallActivity.class);
        return PendingIntent.getActivity(context, 0, intent, 0);
    }

    /**
     * M: Creates the intent to start a RTT downgrade request
     */
    private PendingIntent createRttDowngradeIntent(Context context) {
        final Intent intent = new Intent(TestCallActivity.ACTION_REMOTE_RTT_DOWNGRADE,
                null, context, TestCallActivity.class);
        return PendingIntent.getActivity(context, 0, intent, 0);
    }
    ...
    /**
     * M: Adds an action to the Notification Builder to add an RTT incoming call.
     */
    private void addRttIncomingAction(Notification.Builder builder, Context context) {
        builder.addAction(0, "Rtt MT", createRttIncomingIntent(context));
    }

    /**
     * M: Adds an action to the Notification Builder to start a RTT upgrade request.
     */
    private void addRttUpgradeAction(Notification.Builder builder, Context context) {
        builder.addAction(0, "Rtt Up", createRttUpgradeIntent(context));
    }

    /**
     * M: Adds an action to the Notification Builder to start a RTT downgrade request.
     */
    private void addRttDowngradeAction(Notification.Builder builder, Context context) {
        builder.addAction(0, "Rtt Down", createRttDowngradeIntent(context));
    }
