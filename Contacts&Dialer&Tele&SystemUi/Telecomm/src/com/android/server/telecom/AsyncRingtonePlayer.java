...
//*/ freeme.zhaozehong, 20170805. for ring gradient
...
public class AsyncRingtonePlayer {
    ...
    private void handlePlay(SomeArgs args) {
        ...
        //*/ freeme.zhaozehong, 20170805. for ring gradient
        mContext = incomingCall != null ? incomingCall.getContext() : null;
        //*/
        args.recycle();
    ...
    private void handleRepeat() {
        ...
        if (mRingtone.isPlaying()) {
            ...
            //*/ freeme.zhaozehong, 20170805. ring gradient
            gradientRing();
            //*/
        }
    ...
    private void handleStop() {
        ...
        //*/ freeme.zhaozehong, 20170805. ring gradient. reset streamVolume
        if (currentVolume != -1 && mAudioManager != null) {
    ...
    private void gradientRing() {
        boolean isWork = mContext != null
                && android.provider.Settings.System.getInt(mContext.getContentResolver(),
                com.freeme.provider.FreemeSettings.System.FREEME_GRADIENT_RING_KEY, 0) == 1;
        if (!isWork)
            return;
        ...
        new Thread() {
            ...
            public void run() {
                float current = 0.1f;
                while (current < 1f && currentVolume != -1) {
                    if (mRingtone.isPlaying()) {
                        mRingtone.setVolume(current);
                        current += 0.1f;
                    } else {
                        mAudioManager.setStreamVolume(AudioManager.STREAM_RING, currentVolume, 0);
