...
//*/ freeme.zhaozehong, 20180428. for freemeOS, sync incall screen and ringer (reference sprd solution)
...
public class Ringer {
    ...
    public boolean startRinging(Call foregroundCall, boolean isHfpDeviceAttached) {
        ...
        //*/ freeme.zhaozehong, 20180428. for freemeOS, sync incall screen and ringer (reference sprd solution)
        if (startSyncRingerAndInCallScreen(foregroundCall, isHfpDeviceAttached)){
            return false;
        }
        //*/
    ...
    private static final int DEFAULT_DELAY_TIMEOUT = 120;
    private static final int DEFAULT_DELAY_TIMES = 40;

    private boolean mSyncOK = false;
    private PowerManager mPm;
    private Call mCallDelay;
    private boolean mIsHfpDeviceAttached;
    private final Handler mHandler = new Handler(Looper.getMainLooper());
    private boolean mIsRunnableRunning = false;
    private boolean mIsRinging = false;

    private void initSyncRingerAndInCallScreen() {
        IntentFilter filter = new IntentFilter();
        filter.addAction(SYNC_INCALL_SCREEN_RINGER);
        mContext.registerReceiver(mSyncReceiver, filter);
        mPm = (PowerManager) mContext.getSystemService(Context.POWER_SERVICE);
    }

    private boolean startSyncRingerAndInCallScreen(Call foregroundCall, boolean isHfpDeviceAttached) {
        mIsRinging = true;
        mCallDelay = foregroundCall;
        mIsHfpDeviceAttached = isHfpDeviceAttached;
        if (!mPm.isInteractive()) {
            if (!mIsRunnableRunning) {
                mIsRunnableRunning = true;
                mHandler.post(mDelayPlayRingerIfNeeded);
                return true;
            }
        }
        return false;
    ...
    private void stopSyncRingerAndInCallScreen() {
        mSyncOK = false;
        mIsRinging = false;
        mIsRunnableRunning = false;
        mHandler.removeCallbacks(mDelayPlayRingerIfNeeded);
    }
    ...
    private Runnable mDelayPlayRingerIfNeeded = new Runnable() {
        ...
        public void run() {
            synchronized (this) {
                count++;
                if (DBG) {
                    Log.d(TAG, "mSyncOK: " + mSyncOK + " count: " + count
                            + " mPm.isInteractive()=" + mPm.isInteractive());
                }
                if (mSyncOK && mPm.isInteractive()) {
                    count = 0;
                    startRinging(mCallDelay, mIsHfpDeviceAttached);
                    mSyncOK = false;
                    mIsRunnableRunning = false;
                } else {
                    if (count <= DEFAULT_DELAY_TIMES) {
                        mHandler.postDelayed(mDelayPlayRingerIfNeeded, DEFAULT_DELAY_TIMEOUT);
                    } else {
                        if (mIsRinging) {
                            startRinging(mCallDelay, mIsHfpDeviceAttached);
                        }
