...
//*/ freeme.zhaozehong, 20180127. for freemeOS, dual sim ringtone
...
public class RingtoneFactory {
    ...
    public Ringtone getRingtone(Call incomingCall) {
        ...
        if(ringtone == null) {
            ...
            //*/ freeme.zhaozehong, 20180127. for freemeOS, dual sim ringtone
            ringtone = getRingtoneExt(userContext, incomingCall, ringtone);
            //*/
        }
        if (ringtone != null) {
    ...
    //*/ freeme.zhaozehong, 20180127. for freemeOS, dual sim ringtone
    private Ringtone getRingtoneExt(Context userContext, Call incomingCall, Ringtone ringtone) {
        if (FreemeOption.FREEME_DUAL_SIM_RINGTONE_SUPPORT) {
            PhoneAccountHandle paHandle = incomingCall.getTargetPhoneAccount();
            Log.v(this, "phoneAccountHandle = " + paHandle);
            ...
            if (paHandle != null) {
                ...
                if (tem != null && tm != null) {
                    try {
                        PhoneAccount account = tm.getPhoneAccount(paHandle);
                        if (account != null) {
                            SubscriptionManager sm = SubscriptionManager.from(mContext);
                            boolean isSim1 = true;
                            if (sm.getActiveSubscriptionInfoCount() > 1) {
                                int subId = tem.getSubIdForPhoneAccount(account);
                                int slotId = sm.getSlotIndex(subId);
                                isSim1 = slotId == 0;
                            }

                            userContext = userContext != null ?
                                    !TextUtils.isEmpty(Settings.System.getString(userContext.getContentResolver(),
                                            isSim1 ? Settings.System.RINGTONE :
                            ...
                            ringtone = RingtoneManager.getRingtone(userContext,
                                    isSim1 ? RingtoneManager.getDefaultUri(RingtoneManager.TYPE_RINGTONE) :
                                            RingtoneManager.getDefaultUri(FreemeRingtoneManager.TYPE_RINGTONE_SIM2));
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
