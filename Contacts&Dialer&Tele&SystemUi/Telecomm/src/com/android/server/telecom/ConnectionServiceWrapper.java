...
public class ConnectionServiceWrapper extends ServiceBinder {
    ...
    // M: For mediatek service adapter
    private boolean mServiceInterfaceRemoved = false;
    ...
    private void addConnectionServiceAdapter(IConnectionServiceAdapter adapter) {
        ...
        /// M: only when the AOSP adapter set, the whole bride process can be known as completed.
        onConnectionBridgeSetup();
    }
    ...
    public void createConnection(final Call call, final CreateConnectionResponse response) {
        ...
        BindCallback callback = new BindCallback() {
            ...
            public void onSuccess() {
                ...
                /// M: [Digits] For plugin to put extra message in this Bundle for IMS.
                ExtensionManager.getDigitsUtilExt().putExtrasForConnectionRequest(extras, call);
    ...
    protected void setMtkServiceInterface(IBinder binder) {
        mServiceInterfaceRemoved = false;
        mMtkServiceInterface = IMtkConnectionService.Stub.asInterface(binder);
        Log.v(this, "Adding MTK Connection Service Adapter directly.");
    ...
    protected void setServiceInterface(IBinder binder) {
        ...
        Log.v(this, "Adding AOSP Connection Service Adapter directly.");
    ...
    protected void removeServiceInterface() {
        ...
        // M: Set flag of service interface has been removed.
        mServiceInterfaceRemoved = true;
    }
    ...
    private final class MtkAdapter extends IMtkConnectionServiceAdapter.Stub {
        ...
        @Override
        public void setIConnectionServiceBinder(IBinder connectionService) {
            synchronized (mLock) {
                Log.w(this, "setIConnectionServiceBinder mServiceInterfaceRemoved = "
                        + mServiceInterfaceRemoved);
                if (!mServiceInterfaceRemoved) {
                    setBinder(connectionService);
                }
                mServiceInterfaceRemoved = false;
            }
        }
        ...
        public void handleCreateConferenceComplete(
                ...
                ParcelableConference conference) throws RemoteException {
            synchronized (mLock) {
    ...
    void createConferenceConnection(Call call, CreateConnectionResponse response) {
        // M: The following are mostly copied from AOSP.
    ...
    private void handleCreateConferenceComplete(
        ...
        if (conference.getState() == Connection.STATE_DISCONNECTED) {
            // A connection that begins in the DISCONNECTED state is an indication of
            // failure to connect; we handle all failures uniformly
            // TODO: ParcelableConference can't carry the DisconnectCause
            removeCall(conferenceId, new DisconnectCause(DisconnectCause.ERROR));
        } else {
            // Successful connection
    ...
    void deviceSwitch(Call call, String number, String deviceId) {
        ...
        if (callId != null && isServiceValid("deviceSwitch")) {
            try {
                logOutgoing("deviceSwitch %s %s %s", callId, number, deviceId);
                mMtkServiceInterface.deviceSwitch(callId, number, deviceId);
            } catch (RemoteException e) {
                Log.e(this, e, "deviceSwitch");
            }
        }
    }
    ...
    void cancelDeviceSwitch(Call call) {
        ...
        if (callId != null && isServiceValid("cancelDeviceSwitch")) {
            try {
                ...
                mMtkServiceInterface.cancelDeviceSwitch(callId);
            } catch (RemoteException e) {
                Log.e(this, e, "cancelDeviceSwitch");
            }
        }
    }
    /// @}
