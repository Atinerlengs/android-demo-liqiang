...
public class InCallController extends CallsManagerListenerBase {
    ...
    private final Call.Listener mCallListener = new Call.ListenerBase() {
        ...
        /// M: Force update for CDMA call state change
        @Override
    ...
    public void bindToServices(Call call) {
        if (mInCallServiceConnection == null) {
            ...
            // M: Check the default dialer only for ECC RTT test. @{
            boolean hasEmergencyCall = mCallsManager.hasEmergencyCall();
            if (hasEmergencyCall && (defaultDialerComponentInfo != null
                    && (defaultDialerComponentInfo.getComponentName().getPackageName())
                            .equals("com.android.server.telecom.testapps"))) {
                systemInCall.setHasEmergency(false);
            } else {
                systemInCall.setHasEmergency(hasEmergencyCall);
            }
            // M: @}
