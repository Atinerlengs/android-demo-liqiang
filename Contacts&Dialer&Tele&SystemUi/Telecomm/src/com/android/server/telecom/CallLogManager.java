...
import com.mediatek.provider.MtkCallLog.Calls;
...
public final class CallLogManager extends CallsManagerListenerBase {
    ...
    private static final String CALL_PULLED = "Call Has Been Pulled by Another Device";
    private static final String ANSWERED_ELSWHERE = "Call Completion Elsewhere";
    private static final String DECLINED = "Another device sent All Devices Busy response";
    private static final String INCOMING_DECLINED = "INCOMING_REJECTED";
    ...
    public void onCallStateChanged(Call call, int oldState, int newState) {
        ...
        boolean isIncomingConference = call.isConference() && call.isIncoming()
                && (call.getParentCall() == null);

        // Log newly disconnected calls only if:
        // 1) It was not in the "choose account" phase when disconnected
        // 2) It is not a Conference call or an incoming conference call
        ...
        if (isNewlyDisconnected &&
                (oldState != CallState.SELECT_PHONE_ACCOUNT &&
                        (!call.isConference() || isIncomingConference) &&
                        !isCallCanceled) &&
                !call.isExternalCall() &&
                (!call.isSelfManaged() ||
            ...
            if (configBundle != null && configBundle.getBoolean(
                ...
                Log.d(TAG, "Call Pull supported");
                String disconnectReason = call.getDisconnectCause().getReason();
                if (disconnectReason != null) {
                  if (!call.isIncoming() &&
                            disconnectReason.equalsIgnoreCase(CALL_PULLED)) {
                       type = OUTGOING_PULLED_AWAY_TYPE;
                       Log.d(TAG, "Call type is OUTGOING_PULLED_AWAY");
                  } else if (disconnectReason.equalsIgnoreCase(DECLINED) ||
                             disconnectReason.equalsIgnoreCase(INCOMING_DECLINED)) {
                       type = DECLINED_EXTERNAL_TYPE;
                       Log.d(TAG, "Call type is Declined remotely");
                  } else if (disconnectReason.equalsIgnoreCase(CALL_PULLED)) {
                       type = INCOMING_PULLED_AWAY_TYPE;
                       Log.d(TAG, "Call type is INCOMING_PULLED_AWAY");
                  }
                }
            }
            /// @}
        ...
        if (isNewlyDisconnected && call.isConferenceInvitation() &&
    ...
    void logConferenceUnconnectedParticipants(Call conferenceCall) {
        ...
        for (String number : conferenceCall.getUnconnectedConferenceInvitationNumbers()) {
    ...
    private void logCall(
        ...
        //*/ freeme.zhaozehong, 20180404. for freemeOS, always show emergency call logs
        okToLogEmergencyNumber = true;
        //*/

        // Don't log emergency numbers if the device doesn't allow it.
        final boolean isOkToLogThisCall = !isEmergency || okToLogEmergencyNumber
                                || ExtensionManager.getCallMgrExt().shouldLogEmergencyNumber();
    ...
    public void onIsConferencedChanged(Call call) {
        ...
        // Behavior: For CS conference call, merge will create new and split will disconnect it.
        // Scenario: Make a call then add another, merge, split and merge again.
        // Issue:    For first one, fail to log child call due to they not disconnected.
        // Change:   Log the child call here when conference split
        //           (To avoid duplicate call entry, check if same entry exist before inert
        //            in MtkCallLog.addEntryAndRemoveExpiredEntries.)
        Log.d(TAG, "ConferenceCallLogId: " + call.getConferenceCallLogId());

        if (!call.isConference() && (call.getParentCall() == null)) {
            Log.i(TAG, "onIsConferencedChanged Log this child call");
            ...
            logCall(call, type, false);
        } else {
            Log.i(TAG, "onIsConferencedChanged No need to handle");
        }
    }
    /// @}
    ...
    private boolean handleConferenceSrvccCallLog(Call childCall, String logNumber) {
        ...
        if (childCall.hasProperty(MtkConnection.PROPERTY_VOLTE)) {
