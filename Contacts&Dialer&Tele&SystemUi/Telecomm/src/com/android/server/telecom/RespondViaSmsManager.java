...
//*/ freeme.zhaozehong, 20180514. redesign send action, the sms maybe to long
...
public class RespondViaSmsManager extends CallsManagerListenerBase {
    ...
    private void rejectCallWithMessage(Context context, String phoneNumber, String textMessage,
        ...
        try {
            ...
            /*/
            ArrayList<String> messages = smsManager.divideMessage(textMessage);
            if (messages != null) {
                int size = messages.size();
                if (size > 0) {
                    boolean isMultiPartNotSupported = isMultiPartMmsNotSupported(context, subId);
                    if (isMultiPartNotSupported) {
                        //In case multipart SMS is not supported
                        for (int msgSegemtSent = 0; msgSegemtSent < size; msgSegemtSent++) {
                            smsManager.sendTextMessage(phoneNumber, null,
                                    messages.get(msgSegemtSent), null, null);
    ...
    //*/ freeme.zhaozehong, 20180514. redesign send action, the sms maybe to long
    private boolean isMultiPartMmsNotSupported(Context context, int subId) {
        ...
        boolean isMultiPartNotSupported = carrierConfig.getBoolean
                        (CarrierConfigManager.KEY_MMS_SEND_MULTIPART_SMS_AS_SEPARATE_MESSAGES_BOOL);
        Log.i(RespondViaSmsManager.this,
                "isMultiPartNotSupported" + isMultiPartNotSupported);
