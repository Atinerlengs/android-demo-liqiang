...
abstract class ServiceBinder {
    ...
    final class Binder2 {
        ...
        void bind(BindCallback callback, Call call) {
            ...
            if (mServiceConnection == null) {
                ...
                // M: Check default binder as mediatek add another binder.
                ...
                synchronized (mLock) {
                    ...
                    if (binder != null) {
                        ...
                        try {
                            ...
                            // M: Add mediatek binder or Google AOSP binder @{
                            setDefaultBinder(binder);
                            // handleSuccessfulConnection();
                            // @}
    ...
    final void unbind() {
        if (mServiceConnection == null) {
            ...
            mServiceConnection = null;
            // M: Set mediatek binder or Google AOSP binder.
            synchronized (mLock) {
                setDefaultBinder(null);
            }
        }
    }
    ...
    /**
     * M: We've insert a mediatek interface pair in the binding procedure.
     * As a result, the really successful connection would happen later than before.
     * Introduce this method to handle the success case in the right time.
     */
    protected void onConnectionBridgeSetup() {
        handleSuccessfulConnection();
    }
    ...
    /**
     * M: Set mediatek service binder.
     */
    protected void setMtkBinder(IBinder binder) {
        if (mMtkBinder != binder) {
            if (binder == null) {
                mMtkBinder = null;
                if (mBinder == null) {
                    Log.w(this, "Google AOSP binder have not been setup.");
                    handleFailedConnection();
                    removeServiceInterface();
                    for (Listener l : mListeners) {
                        l.onUnbind(this);
    ...
    /**
     * M: Set default service binder.
     */
    private void setDefaultBinder(IBinder binder) {
    ...
    /**
     * M: Check whether binder is null.
     */
    private void checkBinderNotNull() {
        if (TelephonyUtil.isPstnComponentName(mComponentName)) {
            Preconditions.checkNotNull(mMtkBinder);
        } else {
            Preconditions.checkNotNull(mBinder);
        }
    }
