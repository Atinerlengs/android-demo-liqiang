...
//*/
    ...
    protected void updateCarrierText() {
        ...
        //*/ freeme.gouzhouping, 20180206. FreemeAppTheme, status bar.
        String divider = mCarrierTextExt.customizeCarrierTextDivider(DIVIDER);
        /*/
        ...
        if (!TextUtils.isEmpty(carrierFinalContent) && carrierFinalContent.indexOf("CMCC") >= 0) {
            carrierFinalContent = carrierFinalContent.replace("— CMCC","");
        ...
        carrierFinalContent = formatCarrierLabel(carrierFinalContent);
        updateTextSize(carrierFinalContent);
        //*/
        setText(carrierFinalContent);

        //*/ freeme.gouzhouping, 20180813. notch.
        if (NotchUtils.hasNotch()) {
            setVisibility(mIsShowCarrier ? View.VISIBLE : View.GONE);
        }
        //*/
    }
    ...
    private class CarrierTextTransformationMethod extends SingleLineTransformationMethod {
        ...
        //*/ freeme.gouzhouping, 20180206. FreemeAppTheme, status bar.
        private char[] REPLACEMENT = new char[] { '\n', '\uFEFF' };
        //*/
    ...
    //*/ freeme.gouzhouping, 20180206. FreemeAppheme, status bar.
    private static final String DIVIDER = "\n";
    private float mCarrierLabelTextSize = getResources().getInteger(R.integer.freeme_carrier_label_size);
    private float mCarrierLabelsTextSize = getResources().getInteger(R.integer.freeme_carrier_labels_size);

    private void updateTextSize(String carrierlabel) {
        if (carrierlabel != null &&  carrierlabel.contains(DIVIDER)) {
            setTextSize(TypedValue.COMPLEX_UNIT_DIP, mCarrierLabelsTextSize);
        } else {
            setTextSize(TypedValue.COMPLEX_UNIT_DIP, mCarrierLabelTextSize);
        }
    }

    @Override
    public void onDarkChanged(Rect area, float darkIntensity, int tint) {
        if (NotchUtils.hasNotch() && mIsNotch) {
            return;
        }
        setTextColor(DarkIconDispatcher.getTint(area, this, tint));
    }

    private String formatCarrierLabel(String carrierText) {
        if (!TextUtils.isEmpty(carrierText)) {
            carrierText = carrierText.replaceAll("\\(.*?\\)|\\—.*|\\（*\\）|\\-.*", "");
        }
        return carrierText;
    }
    //*/
    ...
    private boolean mIsShowCarrier;

    public void setNotch(boolean isNotch) {
        mIsNotch = isNotch;
    }

    public void setShowCarrier(boolean isShow) {
        mIsShowCarrier = isShow;
    }

    public void updateCarrierVis() {
        if (NotchUtils.hasNotch()) {
            updateCarrierText();
        }
    }
