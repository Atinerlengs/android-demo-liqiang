...
public class NavigationBarInflaterView extends FrameLayout
    ...
    protected String getDefaultLayout() {
        ...
        if (FreemeOption.Navigation.supports(FreemeOption.Navigation.FREEME_NAVIGATION_COLLAPSABLE)) {
            mNavigationBarStyle = FreemeNavigation.Util.getStyle(mContext.getContentResolver(),
                    FreemeNavigation.Style.BACK_LEFT | FreemeNavigation.Style.BACK_RIGHT);
            mNavigationBarCollapsable = Settings.System.getIntForUser(mContext.getContentResolver(),
                    FreemeSettings.System.FREEME_NAVIGATION_BAR_COLLAPSABLE, 1, UserHandle.USER_CURRENT) != 0;
    ...
    private ContentObserver mNavigationBarStyleObserver = new ContentObserver(new Handler()) {
        @Override
        public void onChange(boolean selfChange) {
            int type = FreemeNavigation.Util.getStyle(mContext.getContentResolver(),
                    FreemeNavigation.Style.BACK_LEFT | FreemeNavigation.Style.BACK_RIGHT);
    ...
    private int computeLayoutType() {
        final boolean rightBack = mNavigationBarStyle == FreemeNavigation.Style.BACK_RIGHT;
        if (mNavigationBarCollapsable) {
            return rightBack ? R.string.config_navBarLayout_collapsable_inversion
                    : R.string.config_navBarLayout_collapsable_normal;
        } else {
            return rightBack ? R.string.config_navBarLayout_inversion : R.string.config_navBarLayout;
        }
    }
    //*/
}
