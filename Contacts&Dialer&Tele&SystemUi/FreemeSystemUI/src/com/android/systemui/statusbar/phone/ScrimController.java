...
public class ScrimController implements ViewTreeObserver.OnPreDrawListener,
    ...
    // M: add value to check scrim animation is launch or not.
    private boolean mAnimationLaunched = false;
    ...
    public ScrimController(LightBarController lightBarController, ScrimView scrimBehind,
        ...
        mLockscreenColor = new GradientColors();
        mLockscreenColor.setMainColor(context.getResources().getColor(R.color.freeme_lockscreen_scrimbehind_color));
        mSuperPowerModeColor = new GradientColors();
        mSuperPowerModeColor.setMainColor(context.getResources().getColor(R.color.freeme_lockscreen_superpowermode_color));
        //*/
    ...
    protected void updateScrims() {
        ...
        if (mNeedsDrawableColorUpdate) {
            ...
            if (mKeyguardShowing) {
                ...
                /*/
                if (mIsShowLockscreen) {
                    mScrimInFront.setColors(mLockscreenColor,false);
                    mScrimBehind.setColors(mLockscreenColor, false);
                } else if (SuperPowerUtils.isSuperPowerModeOn(mScrimBehind.getContext())) {
                    mScrimInFront.setColors(mSuperPowerModeColor, true /* animated */);
                    mScrimBehind.setColors(mSuperPowerModeColor, false);
                } else {
                    mScrimInFront.setColors(mLockColors, true /* animated */);
                    mScrimBehind.setColors(mLockColors, true /* animated */);
                }
    ...
    private void updateScrimNormal() {
        ...
        // M: if the scrim animation launching and frac value if > 0,
        // scrim view alpha will changed, UI blocked after scrim
        // animation end.
        // So we add a condition fix this error.
        if (mAnimationLaunched) {
            frac = 0;
        }
    ...
    private void startScrimAnimation(final View scrim, float target) {
        ...
        anim.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                dispatchScrimsVisible();
                mAnimationLaunched = false;
            }
        });
        anim.start();
        mAnimationLaunched = true;
        if (mAnimateKeyguardFadingOut) {
    ...
    private GradientColors mLockscreenColor;
    private GradientColors mSuperPowerModeColor;

    public void setLockscreenShowStatus(boolean vis) {
        mIsShowLockscreen = vis;
        updateScrims();
    }
    //*/
}
