...
//*/ freeme.xiaohui,20180515, FreemeLockscreen
import com.freeme.keyguard.FreemeKeyguardStatusViewConfig;
import com.freeme.lockscreen.FreemeLockscreenGestureHelper;
//*/
...
public class NotificationPanelView extends PanelView implements
    ...
    protected void loadDimens() {
        ...
        //*/ freeme.xiaohui, 20180523, FreemeLockscreen
        mMinSlop = getResources().getInteger(R.integer.magazine_kg_clock_date_min_slop);
        //*/
    }
    ...
    public void onOverlayChanged() {
        ...
        mKeyguardBottomArea = (KeyguardBottomAreaView) LayoutInflater.from(mContext).inflate(
                //*/ freeme.gouzhouping, 20180129. FreemeAppTheme, keyguard view.
                R.layout.freeme_keyguard_bottom_area,
                /*/
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mQsExpanded && mQsFullyExpanded) {
            ...
            //*/ freeme.gouzhouping, 20180120. FreemeAppTheme, qs container.
            if (!mKeyguardShowing && mQs != null && mQs.isShowingDetail()) {
                setQsExpansion(mQsMaxExpansionHeight + mLastOverscroll);
            } else
            //*/
    ...
    private void positionClockAndNotifications() {
        ...
        //*/ freeme.gouzhouping, 20180410. FreemeLockscreen.
        int h = mClockPositionResult.clockY;
        if (isKeyguardStatusViewBottom()) {
            h = (int)(getHeight() - mKeyguardStatusView.getHeight()  - mMinSlop);
        }
        //*/
        if (mStatusBarState != StatusBarState.KEYGUARD) {
            ...
            //*/ freeme.xiaohui, 20180621, FreemeLockscreen
            if (mQsExpanded) {
                return ;
            }
            //*/
            ...
            if (animate || mClockAnimator != null) {
                //*/ freeme.xiaohui,20180523, FreemeLockscreen
                startClockAnimation(mClockPositionResult.clockX, h);
                /*/
    ...
    public int computeMaxKeyguardNotifications(int maximum) {
        ...
        //*/ freeme.xiaohui, 20180626, FreemeLockscreen
        if (isKeyguardStatusViewBottom()) {
            availableSpace = mNotificationStackScroller.getHeight() - minPadding - shelfSize
                - (getHeight() - mKeyguardStatusView.getY() + shelfSize);
        }
        //*/
        int count = 0;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if ((!mIsExpanding || mHintAnimationRunning)
                ...
                //*/ freeme.xiaohui,20180530, FreemeLockscreen
                && !mLockscreenShowMode
                //*/
                && !mDozing) {
        ...
        //*/ freeme.xiaohui,20180429, FreemeLockscreen
        if (mStatusBar.getBarState() == StatusBarState.KEYGUARD  && (!mQsExpanded)) {
            if (mStatusBar.getLockscreenHostManager().enableLockscreenPanelTouch()) {
                if (mLockscreenShowMode) {
                    requireLockscreenPowerOn(1000L);
                    return false;
                }
                mLockscreenGestureHelper.setDislpaySize(getHeight(), getWidth());
                mLockscreenGestureHelper.onTouch(event);
                if (mLockscreenGestureHelper.isVerticalTouchUsed()) {
                    return false;
                }
            }
        }
        ...
        /*/ freeme.xiaohui, 20180417, FreemeLockscreen
        return mDozing ? handled :true;
        /*/
        return (mDozing || mLockscreenShowMode) ? handled : true;
        //*/
    }
    ...
    public void setBarState(int statusBarState, boolean keyguardFadingAway,
        ...
        if (keyguardShowing) {
            mKeyguardBottomArea.updateIndicationLayout();
            updateLockscreenShowModeStatus();
        }
        //*/
    }
    ...
    private void setQsExpansion(float height) {
        ...
        mStatusBar.onQsExpanded();
        //*/

        //*/ freeme.gouzhouping, 20180307. FreemeAppTheme, blur view keyguard.
        if (mStatusBarState != StatusBarState.SHADE) {
            mStatusBar.updateKeyguardBlurView(height);
        }
        //*/

        //*/ freeme.xiaohui,20180426, FreemeLockscreen
        if (mLockscreenShowMode && (height > mStatusBarMinHeight)) {
            setLockscreenShowMode(false);
        }
        //*/
    }
    ...
    protected void onExpandingStarted() {
        ...
        //*/ freeme.gouzhouping, 20180626. weather conditions.
        WeatherUtils.getInstance(getContext()).query();
        //*/
    }
    ...
    protected float getPeekHeight() {
        /*/ freeme.gouzhouping, 20180118. optimize animation of clicking statusbar.
        if (mNotificationStackScroller.getNotGoneChildCount() > 0) {
    ...
    //*/ freeme.xiaohui, 20180419, FreemeLockscreen
    private boolean mLockscreenShowMode;
    private int mConfigStatusBar = View.VISIBLE;
    private int mConfigBottom = View.VISIBLE;
    private int mMinSlop;

    private FreemeLockscreenGestureHelper mLockscreenGestureHelper = new FreemeLockscreenGestureHelper(mContext) {
        @Override
        public void isHorizontalTarget() {
            setLockscreenShowMode(true);
        }

        @Override
        public void horizontalTouchAnimation(float alpha) {
            setKeyguardBottomViewAlpha(alpha);
            setKeyguardStatusViewAlpha(alpha);
        }
    };

    public void setKeyguardStatusViewAlpha(float alpha) {
        mKeyguardStatusView.setAlpha(alpha);
    }

    public void setKeyguardBottomViewAlpha(float alpha) {
        mKeyguardBottomArea.setAlpha(alpha);
    }

    public void setKeyguardImageTitle(String title) {
        mKeyguardStatusView.setImageTitle(title);
    }

    public void setLockscreenShowMode(boolean mode) {
        if (mStatusBarState == StatusBarState.KEYGUARD) {
            mLockscreenShowMode = mode;
        } else {
            mLockscreenShowMode = false;
        }
        updateLockscreenShowModeStatus();
    }

    private void updateLockscreenShowModeStatus() {
        setKeyguardLockscreenShow(mLockscreenShowMode);
        mStatusBar.getNotificationScrollLayout().setVisibility(mLockscreenShowMode ? View.INVISIBLE : View.VISIBLE);
        mStatusBar.getLockscreenHostManager().setLockscreenConfigShow(mLockscreenShowMode);
        updateFaceService();
    }

    public void setKeyguardLockscreenShow(boolean mode) {
        if (mode || (mStatusBarState != StatusBarState.KEYGUARD)) {
            setKeyguardStatusView(View.INVISIBLE);
            setKeyguardBottomView(View.INVISIBLE);
        } else {
            setKeyguardStatusView(mConfigStatusBar);
            setKeyguardBottomView(mConfigBottom);
        }
    }

    public void setKeyguardStatusView(int vis) {
        if (vis == View.VISIBLE) {
            mKeyguardStatusView.updateImageTitle();
        }
        setKeyguardStatusViewStatus(mKeyguardStatusView, vis);
    }

    public void setKeyguardBottomView(int vis) {
        setKeyguardStatusViewStatus(mKeyguardBottomArea, vis);
    }

    private void requireLockscreenPowerOn(long time) {
        mPowerManager.wakeUp(time, "FreemeLockscreen");
    }

    private void setKeyguardStatusViewStatus(View v, int vis) {
        v.setVisibility(vis);
    }

    public void resetLockscreenShowStatus() {
        mLockscreenShowMode = false;
        updateLockscreenShowModeStatus();
    }

    public void setPanelConfigFromLockscreen(int vStatus, int vBottom) {
        mConfigStatusBar = vStatus;
        mConfigBottom = vBottom;
    }

    private boolean isKeyguardStatusViewBottom() {
        return FreemeKeyguardStatusViewConfig.sConfig
                == FreemeKeyguardStatusViewConfig.CONFIG_KG_STATUSVIEW_BOTTOM;
    }

    private void updateFaceService() {
        if (mStatusBar.getLockscreenHostManager().isMonopolyMode()) {
            return;
        }
        ...
        if (mLockscreenShowMode) {
            ...
            updateMonitor.updateFaceService(true);
        }
    }
    //*/
}
