...
public class NotificationHeaderUtil {
    ...
    private void sanitizeHeader(NotificationHeaderView rowHeader) {
        ...
        if (!hasVisibleText
                && rowHeader.findViewById(com.android.internal.R.id.icon).getVisibility() != View.VISIBLE) {
            if (mIconMarginStart == Integer.MIN_VALUE) {
                mIconMarginStart = getViewMarginStart(rowHeader.findViewById(
                        com.android.internal.R.id.icon));
            }
            setViewMarginStart(time, mIconMarginStart);
        }
        //*/
    ...
    //*/ freeme.biantao, 20180207. Theme.Freeme 8.1 - notification.
    private int mIconMarginStart = Integer.MIN_VALUE;

    private static void setViewMarginStart(View v, int start) {
        if (v == null) {
            return;
        }
        ...
        if (lp instanceof android.view.ViewGroup.MarginLayoutParams) {
            mlp = (android.view.ViewGroup.MarginLayoutParams) lp;
        } else {
            mlp = new android.view.ViewGroup.MarginLayoutParams(lp);
        }
        mlp.setMarginStart(start);
        v.setLayoutParams(mlp);
    }

    private static int getViewMarginStart(View v) {
        if (v == null) {
            return 0;
        }
        ...
        android.view.ViewGroup.LayoutParams lp = v.getLayoutParams();
        if (lp instanceof android.view.ViewGroup.MarginLayoutParams) {
            return ((android.view.ViewGroup.MarginLayoutParams) lp).getMarginStart();
        }
        return 0;
    }
    //*/
