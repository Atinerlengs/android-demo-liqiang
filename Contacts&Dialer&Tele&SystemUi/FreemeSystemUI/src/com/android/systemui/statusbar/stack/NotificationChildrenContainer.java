...
public class NotificationChildrenContainer extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (divider.getVisibility() != GONE) {
                divider.layout(0, 0, getWidth(), mDividerHeight);
            }
            /*/
    ...
    private void updateDividerVisibility() {
        final int dividerCount = mDividers.size();
        if (dividerCount > 0) {
            mDividers.get(0).setVisibility(GONE);
            for (int i = 1; i < dividerCount; i++) {
                mDividers.get(i).setVisibility(VISIBLE);
            }
        }
    }
    //*/
