...
public class ExpandableNotificationRow extends ActivatableNotificationView
    ...
    public void onNotificationUpdated() {
        ...
        setStyleInGroup(mIsStyleInGroup);
        //*/
    }
    ...
    void updateShelfIconColor() {
        ...
        /*/ freeme.gouzhouping, 20180831. notification icon.
        boolean colorize = !isPreL || NotificationUtils.isGrayscale(expandedIcon,
        /*/
        boolean colorize = NotificationUtils.isGrayscale(expandedIcon,
        //*/
    ...
    public void setIsChildInGroup(boolean isChildInGroup, ExpandableNotificationRow parent) {;
        ...
        updateIconVisibilities();
        //*/ freeme.biantao, 20180207. Theme.Freeme 8.1 - notification.
        if (isChildInGroup && !mIsStyleInGroup) {
            setStyleInGroup(isChildInGroup);
        }
        //*/
    }
    ...
    //*/ freeme.biantao, 20180207. Theme.Freeme 8.1 - notification.
    private boolean mIsStyleInGroup;

    public boolean isStyleInGroup() {
        return mIsStyleInGroup;
    }

    public void setStyleInGroup(boolean isInGroup) {
        if (isInGroup) {
            hideAllBackground();
        } else {
            showAllBackground();
        }
        setChildrenMargins(isInGroup);
        mIsStyleInGroup = isInGroup;
    }

    public void hideAllBackground() {
        setNormalBackground(null);

        if (mPublicLayout != null) {
            mPublicLayout.setHeaderBackground(null);
        }
        if (mPrivateLayout != null) {
            mPrivateLayout.setHeaderBackground(null);
        }
    }

    private void showAllBackground() {
        if (mHeadsUpManager.getBar().shouldPeek(mEntry)) {
            setNormalBackground(mContext.getDrawable(R.drawable.freeme_heads_notification_material_bg));
        } else {
            setNormalBackground(mContext.getDrawable(R.drawable.notification_material_bg));
        }

        if (mPublicLayout != null || mPrivateLayout != null) {
            final Drawable bg = mContext.getDrawable(
                    com.android.internal.R.drawable.notification_freeme_header_background);
            if (mPublicLayout != null) {
                mPublicLayout.setHeaderBackground(bg);
            }
            if (mPrivateLayout != null) {
                mPrivateLayout.setHeaderBackground(bg);
            }
        }
    }

    private void setChildrenMargins(boolean isInGroup) {
        int itemMarginStart = 0;
        int itemMarginEnd = 0;
        if (!isInGroup) {
            Resources res = mContext.getResources();
            itemMarginStart = res.getDimensionPixelSize(R.dimen.notification_item_margin_start);
            itemMarginEnd = res.getDimensionPixelSize(R.dimen.notification_item_margin_end);
        }
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (lp instanceof MarginLayoutParams) {
                ...
                mlp.setMarginStart(itemMarginStart);
                mlp.setMarginEnd(itemMarginEnd);
