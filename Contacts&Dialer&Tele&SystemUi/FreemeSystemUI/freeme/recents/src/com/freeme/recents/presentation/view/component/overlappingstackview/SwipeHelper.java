...
public class SwipeHelper {
    ...
    private void snapChild(final View view, float velocity) {
        ...
        anim.setDuration(duration);
        anim.setInterpolator(Config.getInstance().linearOutSlowInInterpolator);
        anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                if (FADE_OUT_DURING_SWIPE && canAnimViewBeDismissed) {
    ...
    private void endSwipe(VelocityTracker velocityTracker) {
        ...
        boolean lockChild =  isValidSwipeDirection(translation)
                && (childSwipedFarEnough || childSwipedFastEnough ) && translation > 0;
        boolean dismissChild = mCallback.canChildBeDismissed(mCurrView)
                && isValidSwipeDirection(translation)
                && (childSwipedFastEnough || childSwipedFarEnough);
        if (lockChild) {
            mCallback.toggleLockStatus(mCurrView);
            // snappity
            mCallback.onDragCancelled(mCurrView);
            snapChild(mCurrView, velocity);
        } else if (dismissChild) {
            // flingadingy
