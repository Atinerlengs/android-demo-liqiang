package com.freeme.recents.presentation.event;
...
import com.freeme.recents.presentation.view.ReferenceCountedTrigger;
...
class EventHandlerMethod {
    ...
    Class<? extends FreemeEventBus.Event> eventType;

    EventHandlerMethod(Method method, Class<? extends FreemeEventBus.Event> eventType) {
        mMethod = method;
        mMethod.setAccessible(true);
        this.eventType = eventType;
    }

    public void invoke(Object target, FreemeEventBus.Event event)
 ...
 * </p>
 */
public class FreemeEventBus extends BroadcastReceiver {

    private static final String TAG = "FreemeEventBus";
    ...
    private static FreemeEventBus sDefaultBus;
     ...
     */
    private FreemeEventBus(Looper looper) {
     ...
     */
    public static FreemeEventBus getDefault() {
        if (sDefaultBus == null)
            synchronized (sLock) {
                if (sDefaultBus == null) {
                    if (DEBUG_TRACE_ALL) {
                        logWithPid("New FreemeEventBus");
                    }
                    sDefaultBus = new FreemeEventBus(Looper.getMainLooper());
                }
            }
        return sDefaultBus;
    }
    ...
    private boolean isValidEventBusHandlerMethod(Method method, Class<?>[] parameterTypes,
        ...
        if (Modifier.isPublic(modifiers) &&
            ...
            if (FreemeEventBus.InterprocessEvent.class.isAssignableFrom(parameterTypes[0]) &&
                    method.getName().startsWith(INTERPROCESS_METHOD_PREFIX)) {
                isInterprocessEventOut.value = true;
                return true;
            } else if (FreemeEventBus.Event.class.isAssignableFrom(parameterTypes[0]) &&
                    method.getName().startsWith(METHOD_PREFIX)) {
                isInterprocessEventOut.value = false;
                return true;
            } else {
                if (DEBUG_TRACE_ALL) {
                    if (!FreemeEventBus.Event.class.isAssignableFrom(parameterTypes[0])) {
