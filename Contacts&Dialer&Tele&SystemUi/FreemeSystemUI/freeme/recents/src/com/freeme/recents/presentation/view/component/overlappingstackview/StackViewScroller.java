...
public class StackViewScroller {
    public interface StackViewScrollerCallbacks {
        void onScrollChanged(float p);
    }

    Config mConfig;
    LayoutAlgorithm mLayoutAlgorithm;
    StackViewScrollerCallbacks mCb;
    ...
    public StackViewScroller(Context context, Config config,
     ...
     * Sets the callbacks
     */
    public void setCallbacks(StackViewScrollerCallbacks cb) {
        mCb = cb;
    }
    ...
    void animateScroll(float curScroll, float newScroll, final Runnable postRunnable) {
        ...
        mScrollAnimator.setDuration(mConfig.childStackScrollDuration);
    ...
    void stopBoundScrollAnimation() {
        Utils.cancelAnimationWithoutCallbacks(mScrollAnimator);
    }
    ...
    int progressToScrollRange(float p) {
        return (int) (p * mLayoutAlgorithm.mStackVisibleRect.width());
    }

    float scrollRangeToProgress(int s) {
        return (float) s / mLayoutAlgorithm.mStackVisibleRect.width();
    }
    ...
    boolean computeScroll() {
        if (mScroller.computeScrollOffset()) {
            float scroll = scrollRangeToProgress(mScroller.getCurrX());
