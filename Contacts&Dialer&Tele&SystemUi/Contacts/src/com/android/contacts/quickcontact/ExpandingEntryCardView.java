...
//*/
    ...
    public ExpandingEntryCardView(Context context, AttributeSet attrs) {
        ...
        /*/
        View expandingEntryCardView = inflater.inflate(R.layout.freeme_expanding_entry_card_view, this);
        //*/
    ...
    private void inflateAllEntries(LayoutInflater layoutInflater) {
        ...
        for (int i = 0; i < mEntries.size(); i++) {
            ...
            for (int j = viewList.size(); j < entryList.size(); j++) {
                isFirst = i == 0 && j == 0;
                isEnd = i == (mEntries.size() - 1) && j == (entryList.size() - 1);
                final int iconVisibility;
                final Entry entry = entryList.get(j);
                // If the entry does not have an icon, mark gone. Else if it has an icon, show
                // for the first Entry in the list only
                ...
                if (isRecentCard) {
                    viewList.add(createStrangerEntryView(layoutInflater, entry));
                } else {
                    viewList.add(createEntryView(layoutInflater, entry, iconVisibility, isFirst, isEnd));
                }
            }
        }
        mAllEntriesInflated = true;
    ...
    /*/
    private View createEntryView(LayoutInflater layoutInflater, final Entry entry,
            int iconVisibility, boolean isFirst, boolean isEnd) {
        final EntryView view = (EntryView) layoutInflater.inflate(
                R.layout.freeme_expanding_entry_card_item, this, false);
        //*/
        ...
        /*/ Freeme.liqiang, 20180122. redesign quick contact.
        // we don't need kind icon any more
        ...
        /*/
        // we use subHeader as header if it's not null at first.
        String entryHeader = entry.getHeader();
        String entrySubHeader = entry.getSubHeader();
        final boolean emptyHeader = TextUtils.isEmpty(entryHeader);
        final boolean emptySubHeader = TextUtils.isEmpty(entrySubHeader);
        if (!emptySubHeader) {
            header.setText(entrySubHeader);
        } else if (!emptyHeader) {
            header.setText(entryHeader);
        //*/
        } else {
        ...
        /*/
        if (!emptyHeader && !emptySubHeader) {
            subHeader.setText(entryHeader);
        //*/
        } else {
        ...
        //*/ freeme.liqiang, 20180122, redesign quick contact
        final ImageView dialIcon = (ImageView) view.findViewById(R.id.dial_icon);
        if (entry.getIconResourceId() != 0) {
            dialIcon.setImageDrawable(getContext().getResources().getDrawable(entry.getIconResourceId()));
            dialIcon.setVisibility(View.VISIBLE);
        }
        //*/
        ...
        //*/ freeme.liqiang, 20180122, redesign quick contact
        final int green_header_title = getResources().getColor(
                R.color.freeme_quick_contact_header_title_color);
        final int black_header_title = getResources().getColor(
                R.color.freeme_quick_contact_dark_title);
        final int gray_header_color = getResources().getColor(
                R.color.freeme_quick_contact_sub_title_color);
        if (null == entry.getIntent()) {
            header.setTextColor(black_header_title);
            subHeader.setTextColor(gray_header_color);
            text.setTextColor(gray_header_color);
        } else {
            header.setTextColor(green_header_title);
            subHeader.setTextColor(gray_header_color);
            text.setTextColor(gray_header_color);
        }
        if (isFirst && isEnd) {//full
            view.setBackgroundResource(R.drawable.freeme_content_full_bg_selector);
        } else {
            if (isFirst) {//top
                view.setBackgroundResource(R.drawable.freeme_content_top_bg_selector);
            } else if (isEnd) {//bottom
                view.setBackgroundResource(R.drawable.freeme_content_bottom_bg_selector);
            } else {//center
                view.setBackgroundResource(R.drawable.freeme_content_center_bg_selector);
            }
        }
        //*/
    ...
    private View createStrangerEntryView(LayoutInflater layoutInflater, final Entry entry) {
        final EntryView view = (EntryView) layoutInflater.inflate(
                R.layout.freeme_expanding_recent_card_item, this, false);
        final TextView recentHeader = view.findViewById(R.id.recent_header);
        final TextView recentSubHeader = view.findViewById(R.id.recent_subheader);
        final TextView recentTime = view.findViewById(R.id.recent_time);
        String recentEntryHeader = entry.getHeader();
        String recentEntrySubHeader = entry.getSubHeader();
        String recentEntryTime = entry.getText();

        final boolean emptyHeader = TextUtils.isEmpty(recentEntryHeader);
        final boolean emptySubHeader = TextUtils.isEmpty(recentEntrySubHeader);
        final boolean emptyTime = TextUtils.isEmpty(recentEntryTime);
        if (!emptyHeader) {
            recentHeader.setText(recentEntryHeader);
        }
        if (!emptySubHeader) {
            recentSubHeader.setText(recentEntrySubHeader);
        }
        if (!emptyTime) {
            recentTime.setText(recentEntryTime);
        }
        return view;
    }

    private static final int CALL_TYPE_ALL = -1; // all call logs
    private void insertCalllogEntriesIntoViewGroup(int type) {
        mEntriesViewGroup.removeAllViews();

        for (View view : getViewsToDisplay(mIsExpanded)) {
            mEntriesViewGroup.addView(view);
        }
        final View callogView = LayoutInflater.from(getContext()).inflate(
                R.layout.freeme_stranger_callllog_item, this, false);
        mEntriesViewGroup.addView(callogView);
        RelativeLayout callLogItem = findViewById(R.id.calllog_item);
        TextView callLogText = findViewById(R.id.calllog);
        callLogText.setText(type != CALL_TYPE_ALL ? mContext.getString(R.string.freeme_recent_call_all_unconnect_calllist)
                : mContext.getString(R.string.freeme_recent_call_all_calllist));
        callLogItem.setOnClickListener(mOnClickListener);
        removeView(mExpandCollapseButton);
        if (mCollapsedEntriesCount < mNumEntries
                && mExpandCollapseButton.getParent() == null && !mIsAlwaysExpanded) {
            mContainer.addView(mExpandCollapseButton, -1);
    ...
    public void recentInitialize(List<List<Entry>> entries, int numInitialVisibleEntries,
                                 boolean isExpanded, boolean isAlwaysExpanded, ExpandingEntryCardViewListener listener,
                                 ViewGroup animationViewGroup, int type) {
        LayoutInflater layoutInflater = LayoutInflater.from(getContext());
        mIsExpanded = isExpanded;
        mIsAlwaysExpanded = isAlwaysExpanded;
        // If isAlwaysExpanded is true, mIsExpanded should be true
        ...
        insertCalllogEntriesIntoViewGroup(type);
    }
    //*/
