...
public class CustomContactListFilterActivity extends BaseEventHandlerActivity implements
    ...
    protected void onCreate(Bundle icicle) {
        ...
        /*/
        mList.setOnItemLongClickListener(new ListItemLongClick());
        //*/
    ...
    //*/ freeme.zhaozehong, 20180627. for freemeOS, replace ContextMenu with AlertDialog
    class ListItemLongClick implements ExpandableListView.OnItemLongClickListener {
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            showBottomMenu(account, child, syncMode);

            return true;
        }
    }

    private final int MENU_ID_SYNC_REMOVE = 10;
    private final int MENU_ID_SYNC_ADD = 20;
    private void showBottomMenu(final AccountDisplay account, final GroupDelta child,
                                final int syncMode) {

        final HashMap<Integer, GroupDelta> dataMap = new HashMap<>();
        final ArrayList<String> resList = new ArrayList<>();
        final ContextMenu menu = new ContextMenuBuilder(this);

        final CharSequence title;
        if (child != null) {
            // showRemoveSync(menu, account, child, syncMode);

            title = child.getTitle(this);
            createMenuData(resList, menu, MENU_ID_SYNC_REMOVE,
                    getString(R.string.menu_sync_remove));
        } else {
            // showAddSync(menu, account, syncMode);

            title = getString(R.string.dialog_sync_add);
            int index = 0;
            for (final GroupDelta delta : account.mUnsyncedGroups) {
                if (!delta.getShouldSync()) {
                    final CharSequence text = delta.getTitle(this);
                    final int id = MENU_ID_SYNC_ADD + index;
                    createMenuData(resList, menu, id,
                            TextUtils.isEmpty(text) ? text.toString() : "");
                    dataMap.put(id, delta);
                }
            }
        }
                ...
                .setItems(menuItems, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (item != null) {
                            ...
                            if (id == MENU_ID_SYNC_REMOVE) {
                                handleRemoveSync(account, child, syncMode, title);
                            } else {
                                GroupDelta delta = dataMap.get(id);
                                if (delta != null) {
                                    if (delta.mUngrouped && syncMode == SYNC_MODE_EVERYTHING) {
                                        account.setShouldSync(true);
                                    } else {
                                        account.setShouldSync(delta, true);
                                    }
                                    mAdapter.notifyDataSetChanged();
                                }
                            }
    ...
    private void createMenuData(final ArrayList<String> resList, final ContextMenu menu,
                                final int menuIdRes, final String menuText) {
        resList.add(menuText);
        menu.add(ContextMenu.NONE, menuIdRes, ContextMenu.NONE, menuText);
    }
    //*/
}
