...
public abstract class ContactEntryListFragment<T extends ContactEntryListAdapter>
    ...
    /*/ freeme.liqiang, 20180308.for FreemeOS remove quick contact badge
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        //*/ freeme.liqiang, 20180301. for FreemeOS Selected Index Scroll
        outState.putBoolean(KEY_VISIBLE_INDEXSCROLL_ENABLED, mVisibleIndexScrollbarEnabled);
        //*/
    ...
    public void restoreSavedState(Bundle savedState) {
        ...
        //*/ freeme.liqiang, 20180301. for FreemeOS Selected Index Scroll
        mVisibleIndexScrollbarEnabled = savedState.getBoolean(KEY_VISIBLE_INDEXSCROLL_ENABLED);
        //*/
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        if (mContactsCountCallBack != null && data != null) {
            mContactsCountCallBack.onShowCount(data.getCount());
        }
        //*/
    }
    ...
    protected void onPartitionLoaded(int partitionIndex, Cursor data) {
        ...
        if (isVisibleIndexScrollbarEnabled()) {
            addIndexScrollView();
        }
        //*/
    ...
    public void setSectionHeaderDisplayEnabled(boolean flag) {
        if (mSectionHeaderDisplayEnabled != flag) {
            ...
            //*/ freeme.liqiang, 20180301. for FreemeOS Selected
            configureVerticalIndexScrollbar();
            //*/
        }
    }
    ...
    protected void onCreateView(LayoutInflater inflater, ViewGroup container) {
        ...
        /*/
        mListView.setFastScrollEnabled(!isSearchMode() && isVisibleScrollbarEnabled());
        //*/
        ...
        //*/ freeme.liqiang, 20180301. for FeemeOS Selected
        mIndexScrollView = (FreemeIndexScrollView) mView.findViewById(R.id.index_scroll_view);
        if (mIndexScrollView != null) {
            mIndexScrollView.setIndexHandleChar(
                    mContext.getResources().getStringArray(R.array.index_handle_chars));
            //mIndexScrollView.setIndexScrollViewTheme(mIndexScrollView.LIGHT_THEME);
            if (shouldSet) {
                mIndexScrollView.setHandleTextTheme(textSize, textInterval, textColor);
            }
            if (shouldSetPopuPosition) {
                mIndexScrollView.setPopupHeight(popuHeight);
            }

            if (mShouldSetScrollExtraBottomLand) {
                mIndexScrollView.setIndexHandleExtraBottomLand(mCustomScrollExtraBottomLand);
            }
            if (mShouldSetScrollExtraBottomPort) {
                mIndexScrollView.setIndexHandleExtraBottomPort(mCustomScrollExtraBottomPort);
            }
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount,
        ...
        //*/ freeme.liqiang, 20180301. for FreemeOS Selected
        if (mIndexScrollView != null) {
            mIndexScrollView.onScroll(null, firstVisibleItem, visibleItemCount, totalItemCount);
        }
        //*/
    }
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        //*/ freeme.liqiang, 20180301. for FreemeOS Selected
        if (mIndexScrollView != null) {
            mIndexScrollView.onScrollStateChanged(null, scrollState);
        }
        //*/
    }
    ...
    public boolean onTouch(View view, MotionEvent event) {
        if (view == mListView) {
            ...
            //*/ freeme.liqiang, 20180301. for FreemeOS Selected
            if (mIndexScrollView != null) {
                mIndexScrollView.hidePopupAndIndex();
            }
            //*/
        }
        return false;
    ...
    //*/ freeme.liqiang, 20180301. for FreemeOS Selected
    private static final String KEY_VISIBLE_INDEXSCROLL_ENABLED = "visibleIndexScrollbarEnabled";
    private FreemeIndexScrollView mIndexScrollView;
    private boolean mVisibleIndexScrollbarEnabled;

    protected void addIndexScrollView() {
        if (mIndexScrollView != null) {
            mIndexScrollView.setOnIndexSelectedListener(
                    new FreemeIndexScrollView.OnIndexSelectedListener() {
                        public void onIndexSelected(String section) {
                            if (mAdapter == null) return;

                            final Object[] sections = mAdapter.getSections();
                            int iSection = (false) ? 1 : 0;

                            // search section pos
                            Object item;
                            final int count = sections.length;
                            for ( /**/; iSection < count; ++iSection) {
                                item = sections[iSection];
                                if (item != null
                                        && section.compareToIgnoreCase(item.toString()) == 0) {
                                    break;
                                } else if (item != null
                                        && section.compareToIgnoreCase(item.toString()) < 0) {
                                    // < catch
                            ...
                            if (iSection >= count) iSection = count - 1;
                            if (section.equals("#")) {
                                iSection = count - 1;
                            }
                            // convert to position
                            int position = mAdapter.getPositionForSection(iSection);
                            if (position < 0) position = 0;
                            // move to position in listview
                            android.util.Log.i(TAG, "ListView.setSelection -> " + position);
    ...
    public boolean showIndexScrollbar() {
        if (mListView != null && mIndexScrollView != null) {
            mIndexScrollView.setVisibility(View.VISIBLE);
            mIndexScrollView.bringToFront();
            return true;
        }
        return false;
    }

    public void hideIndexScrollbar() {
        if (mListView != null && mIndexScrollView != null) {
            mIndexScrollView.setVisibility(View.GONE);
        }
    }

    public void setVisibleIndexScrollbarEnabled(boolean enabled) {
        setVisibleIndexScrollbarEnabled(enabled, false);
    }

    public void setVisibleIndexScrollbarEnabled(boolean enabled, boolean force) {
        if (mVisibleIndexScrollbarEnabled != enabled || force) {
            mVisibleIndexScrollbarEnabled = enabled;
            configureVerticalIndexScrollbar();
        }
    }

    public boolean isVisibleIndexScrollbarEnabled() {
        return mVisibleIndexScrollbarEnabled;
    }

    private void configureVerticalIndexScrollbar() {
        boolean hasIndexScrollbar = isVisibleIndexScrollbarEnabled() && isSectionHeaderDisplayEnabled();
        if (mListView != null) {
            mListView.setFastScrollEnabled(false);
            mListView.setFastScrollAlwaysVisible(false);
            mListView.setScrollBarStyle(ListView.SCROLLBARS_OUTSIDE_OVERLAY);
            mListView.setVerticalScrollBarEnabled(!hasIndexScrollbar);
            boolean isShownIndexScrollBar = false;
            if (hasIndexScrollbar) {
                isShownIndexScrollBar = showIndexScrollbar();
            } else {
                hideIndexScrollbar();
            }

            int padding = getResources().getDimensionPixelOffset(R.dimen.freeme_indexview_handle_width);
            boolean isScrollbarLeft = mVerticalScrollbarPosition == View.SCROLLBAR_POSITION_LEFT;
            int leftPadding = isScrollbarLeft ? padding : mListView.getPaddingLeft();
            int rightPadding = isScrollbarLeft ? mListView.getPaddingRight() : padding;
            if (!isShownIndexScrollBar) {
                leftPadding = mListView.getPaddingLeft();
                rightPadding = mListView.getPaddingRight();
            }
    ...
    public int textInterval = 0;
    public boolean shouldSet = false;
    public int textColor = 0;

    public void setTextSize(int textSize, int textInterval, int textColor, boolean shouldSet) {
    ...
    private boolean shouldSetPopuPosition = false;

    public void setPopuPosition(int popuHeight, boolean shouldSet) {
        this.popuHeight = popuHeight;
        this.shouldSetPopuPosition = shouldSet;
        if (mIndexScrollView != null) {
            mIndexScrollView.setPopupHeight(popuHeight);
        }
    }

    public void hidePopuAndIndex() {
        if (mIndexScrollView != null) {
            mIndexScrollView.hidePopupAndIndex();
        }
    }

    private int mCustomScrollExtraBottomLand;
    private boolean mShouldSetScrollExtraBottomLand;

    public void setCustomScrollExtraBottomLand(int extraBottomLand, boolean shouldExtraBottomLand) {
        this.mCustomScrollExtraBottomLand = extraBottomLand;
        this.mShouldSetScrollExtraBottomLand = shouldExtraBottomLand;
        if (mShouldSetScrollExtraBottomLand && mIndexScrollView != null) {
            mIndexScrollView.setIndexHandleExtraBottomLand(extraBottomLand);
        }
    }

    private int mCustomScrollExtraBottomPort;
    private boolean mShouldSetScrollExtraBottomPort;

    public void setCustomScrollExtraBottomPort(int extraBottomPort, boolean shouldExtraBottomPort) {
        this.mCustomScrollExtraBottomPort = extraBottomPort;
        this.mShouldSetScrollExtraBottomPort = shouldExtraBottomPort;
        if (mShouldSetScrollExtraBottomPort && mIndexScrollView != null) {
            mIndexScrollView.setIndexHandleExtraBottomPort(extraBottomPort);
        }
    }
    ...
    private ContactsCountCallBack mContactsCountCallBack;

    public void setContactsCountCallBack(ContactsCountCallBack contactsCountCallBack) {
        this.mContactsCountCallBack = contactsCountCallBack;
    }
