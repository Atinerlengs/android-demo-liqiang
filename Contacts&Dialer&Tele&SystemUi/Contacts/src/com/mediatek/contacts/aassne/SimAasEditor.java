...
public class SimAasEditor {
        ...
        if (AccountTypeUtils.isUsimOrCsim(accountType)) {
            ...
            for (Object obj : anrList) {
                ...
                if (!TextUtils.isEmpty(anr.additionalNumber)) {
                    Log.d(TAG, "[buildAnrOperation] additionalNumber=" + anr.additionalNumber +
                            " aas=" + anr.aasIndex);
                    ...
                    builder.withValue(Data.DATA2, Anr.TYPE_AAS);
                    int aasIndex = SimAasSneUtils.getAasIndexByName(anr.aasIndex,
    ...
    public void updateValuesforCopy(Uri sourceUri, int subId, String accountType,
            ...
            if (c != null && c.moveToFirst()) {
                do {
                    ...
                    if (Phone.CONTENT_ITEM_TYPE.equals(mimeType)) {
                        ...
                        Log.d(TAG, "[updateValuesforCopy] simAnrNum:" + entry.additionalNumber);

                        entry.aasIndex = mContext.getString(Phone
                                .getTypeLabelResource(c.getInt(4)));
                        Log.d(TAG, "[updateValuesforCopy] aasIndex:" + entry.aasIndex);
        ...
        for (int i = 0; i < count; i++) {
            ...
            Log.d(TAG, "[updateValuesforCopy] additionalNumber:" + entry.additionalNumber);
        ...
        for (int countIndex = 0; countIndex < kindCount; countIndex++) {
            ...
            if (Phone.CONTENT_ITEM_TYPE.equals(mimeType)) {
                ...
                if (isAdditionalNumber(cv)) {
                    ...
                    Log.d(TAG, "[prepareNewAnrList] additionalNumber:" + addPhone.additionalNumber
        ...
        for (int oldIndex = 0; oldIndex < oldCount; oldIndex++) {
            ...
            if (Phone.CONTENT_ITEM_TYPE.equals(mimeType)) {
                ...
                if (isAdditionalNumber(cv)) {
                    ...
                    Log.d(TAG, "[prepareOldAnrList] additionalNumber:" + addPhone.additionalNumber
                            + ",aasIndex: " + addPhone.aasIndex);
            ...
            for (Object obj : anrsList) {
                ...
                if (!TextUtils.isEmpty(additionalNumber)) {
                    ...
                    if (!Pattern.matches(SIM_NUM_PATTERN,
                            MtkPhoneNumberUtils.extractCLIRPortion(additionalNumberToInsert))) {
                ...
                values.put("aas" + SimAasSneUtils.getSuffix(count),
                        getAasIndexFromIndicator(anr.aasIndex));
                count++;
        ...
        if (AccountTypeUtils.isUsimOrCsim(accountType)) {
            ...
            for (Anr anr : anrsList) {
                ...
                if (!TextUtils.isEmpty(anr.additionalNumber)) {
                    ...
                    if (!Pattern.matches(SIM_NUM_PATTERN, MtkPhoneNumberUtils
    ...
    private boolean updateAnrToDb(String accountType, ContentResolver resolver,
        ...
        if (AccountTypeUtils.isUsimOrCsim(accountType)) {
            String whereClause = Data.RAW_CONTACT_ID + "=\'" + rawId
                    ...
                    + MtkContactsContract.DataColumns.IS_ADDITIONAL_NUMBER
                    + "=1" + " AND " + Data._ID + "=?";
            Log.d(TAG, "[updateAnrInfoToDb] whereClause:"
                    + whereClause);
                ...
                for (Object obj : mAnrsList) {
                    ...
                    if (!TextUtils.isEmpty(anr.additionalNumber)) {
                        Log.d(TAG, "[updateOperationList] additionalNumber="
                                + anr.additionalNumber + ",aas=" + anr.aasIndex);
    ...
    public boolean resetSubHeader(int subId, int type) {
        Log.d(TAG, "[resetSubHeader] subId = " + subId);
        if (subId == -1) {
            Log.d(TAG, "[resetSubHeader] Phone contact");
            return false;
        }
        String accountType = AccountTypeUtils.getAccountTypeBySub(subId);
        if (AccountTypeUtils.isUsimOrCsim(accountType)) {
            if (AccountTypeUtils.isAasPhoneType(type)) {
                Log.d(TAG, "[resetSubHeader] USIM additional number");
                return true;
            }
        }
        Log.d(TAG, "[resetSubHeader] Account is SIM ");
        return false;
    }
    //*/
}
