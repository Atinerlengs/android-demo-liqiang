...
public class AasTagActivity extends Activity {
    ...
    /*/ freeme.linqingwei, 20180321. redesign aas tag.
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG, "[onCreate]");
        /*[ALPS03506565] Check permission.
         * Refresh UsimPhbInfoMap if retrurn from permission activity*/
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (mAasAdapter.isMode(AasTagInfoAdapter.MODE_NORMAL)) {
            ...
            setTitle(R.string.aas_custom_title);
        } else {
            inflater.inflate(R.menu.custom_edit_menu, menu);
            final int checkedCount = mAasAdapter.getCheckedItemCount();
            final int totalCount = mAasAdapter.getCount();
            // delete
            MenuItem item = menu.findItem(R.id.menu_delete);
            item.setEnabled(checkedCount > 0);
            // selection
            item = menu.findItem(R.id.menu_select_all);
            item.setTitle(checkedCount >= totalCount ?
                R.string.menu_select_none : R.string.menu_select_all);
            // title
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menu_select_all:
                mAasAdapter.setAllChecked(
                        mAasAdapter.getCheckedItemCount() != mAasAdapter.getCount());
    ...
    public EditTextDoneListener getEditTextDoneListener(String dialogTag) {
        Log.d(TAG, "[getEditTextDoneListener] dialogTag = " + dialogTag);
    ...
    final private class NewAlpahTagListener implements EditTextDoneListener {
        ...
        public void onEditTextDone(String text) {
            Log.d("NewAlpahTagListener", "[onEditTextDone] text = " + text);
    ...
    final private class EditAlpahTagListener implements EditTextDoneListener {
        ...
        @Override
        public void onEditTextDone(String text) {
            Log.e("EditAlpahTagListener", "[onEditTextDone] text=" + text
                    + ", mAlphaTag.getAlphaTag()=" + mAlphaTag.getAlphaTag());
    ...
    final private class EditAssertListener implements AlertConfirmedListener {
        ...
        public void onMessageAlertConfirmed(String text) {
            Log.d("EditAssertListener", "[onMessageAlertConfirmed] text = " + text);
    ...
    private class ListItemLongClickListener implements AdapterView.OnItemLongClickListener {
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
            if (mAasAdapter.isMode(AasTagInfoAdapter.MODE_NORMAL)) {
                mAasAdapter.setMode(AasTagInfoAdapter.MODE_EDIT);
            }
            mAasAdapter.updateChecked(position);
            invalidateOptionsMenu();
