...
public class ImportExportItem extends LinearLayout {
    ...
    /*/ freeme.liqiang, 20180309. for FreemeOS contact import and export
    ...
    private boolean isExternalStoragePath(String path) {
        if (path == null || path.isEmpty()) {
            Log.d(TAG, "[isExternalStoragePath]empty path: " + path);
            return false;
        }
        ...
        if (storageVolumeList != null) {
            for (StorageVolume volume : storageVolumeList) {
                ...
                Log.d(TAG, "[isExternalStoragePath]isExternal=" + isExternal + ", path=" + p);
