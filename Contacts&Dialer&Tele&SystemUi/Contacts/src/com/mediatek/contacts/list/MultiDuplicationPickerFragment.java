...
public class MultiDuplicationPickerFragment extends MultiBasePickerFragment {
    ...
    public void onOptionAction() {
        ...
        //*/ freeme.zhaozehong, 20180529. for freemeOS, UI redesign
        boolean isImmediately = getActivity().getIntent()
                .getBooleanExtra("do_immediately", true);
        if (!isImmediately) {
            final long[] checkedIds = getCheckedItemIds();
            if (checkedIds == null || checkedIds.length <= 0) {
                Log.w(TAG, "[onOptionAction]listItemCacher is empty,return.");
                getActivity().setResult(200);//NO CHECKED CONTACTS
                getActivity().finish();
                return;
            }
            Intent ret = new Intent();
            ret.putExtra("checkedIds", checkedIds);
    ...
    public MultiDuplicationPickerFragment(){
