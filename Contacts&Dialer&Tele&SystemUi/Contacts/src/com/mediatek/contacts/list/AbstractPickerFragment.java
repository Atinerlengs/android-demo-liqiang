...
public abstract class AbstractPickerFragment extends
    ...
    protected void onCreateView(LayoutInflater inflater, ViewGroup container) {
        ...
        /*/
        mEmptyView = (FreemeEmptyContentView) getView().findViewById(R.id.empty_list_view);
        mEmptyView.setImage(R.drawable.freeme_empty_icon_contacts);
        mEmptyView.setDescription(R.string.freeme_empty_contacts);
        getView().setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                AbstractPickerFragment.this.onTouch(getListView(), event);
                return false;
            }
        });
        //*/
   ...
   @Override
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        if (mEmptyView != null) {
            if (getAdapter().isSearchMode()) {
                mEmptyView.setDescription(R.string.freeme_no_match_contact);
            } else {
                mEmptyView.setDescription(R.string.freeme_empty_contacts);
            }
        ...
        if (count <= 0) {
            getListView().setFastScrollEnabled(false);
            getListView().setFastScrollAlwaysVisible(false);
        } else if (!getAdapter().isSearchMode() && isVisibleScrollbarEnabled()) {
    ...
    protected void updateSelectedItemsView(int checkedItemsCount) {
        ...
        //*/ freeme.liqiang, 20180301. for FreemeOS Selected
        if (mCallBack != null) {
            mCallBack.onCheckedItemCount(checkedItemsCount, mCountNumbers);
        }
        //*/
    ...
    private void setOkButtonStatus(boolean enable) {
        ...
        if (optionView != null) {
            if (enable) {
                ...
                /*/
                optionView.setTextColor(getResources().getColor(R.color.tab_title_text_color));
                //*/
            } else {
    ...
    //*/ freeme.liqiang, 20180301. for FreemeOS Selected
    private CheckedItemCountCallBack mCallBack;

    public void setCheckedItemCountCallBack(CheckedItemCountCallBack callBack) {
        mCallBack = callBack;
    }

    interface CheckedItemCountCallBack {
        void onCheckedItemCount(int count, int size);
    }
    //*/
}
