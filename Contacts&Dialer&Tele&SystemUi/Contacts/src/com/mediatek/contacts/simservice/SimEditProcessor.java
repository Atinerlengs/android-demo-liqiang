...
public class SimEditProcessor extends SimProcessorBase {
    ...
    public void doWork() {
        ...
        Log.d(TAG, "[dowork] sName = " + sName + ", sPhone =" + sPhone
                + ", sOtherPhone = " + sOtherPhone + ", email =" + sEmail);
        ...
        Log.d(TAG, "[dowork]sName = " + sName + ", sPhone =" + sPhone
                + ", additionalNumberBuffer[0] = " + additionalNumberBuffer[0]
                + ", sOtherPhone = " + sOtherPhone + ", email =" + sEmail);
    }
    ...
    private boolean isPhoneNumberInvaild() {
        ...
        if (!TextUtils.isEmpty(sPhone)) {
            ...
            if (!Pattern.matches(SIM_NUM_PATTERN,
                    MtkPhoneNumberUtils.extractCLIRPortion(sAfterPhone))) {
                return true;
            }
        }
        ...
        if (!TextUtils.isEmpty(sOtherPhone)) {
            ...
            if (!Pattern.matches(SIM_NUM_PATTERN,
                    MtkPhoneNumberUtils.extractCLIRPortion(sAfterOtherPhone))) {
                return true;
            }
        }
        return false;
    ...
    private boolean isSIMUpdateValuesInvalid() {
        if (TextUtils.isEmpty(mUpdateName) && TextUtils.isEmpty(mUpdatephone)) {
            ...
            if (!Pattern.matches(SIM_NUM_PATTERN,
                    MtkPhoneNumberUtils.extractCLIRPortion(mUpdatephone))) {
                mNumberInvalid = true;
            }
        }
    ...
    private boolean isUSIMUpdateValuesInvalid() {
        ...
        if (!TextUtils.isEmpty(mUpdateAdditionalNumber)) {
            if (!Pattern.matches(SIM_NUM_PATTERN,
                    MtkPhoneNumberUtils.extractCLIRPortion(mUpdateAdditionalNumber))) {
