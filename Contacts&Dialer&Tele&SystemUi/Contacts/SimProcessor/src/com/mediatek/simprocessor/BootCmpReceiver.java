...
public class BootCmpReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Log.d(TAG, "[onReceive] action=" + action + ", the current user is: "
                + UserHandle.myUserId() + " isRunning: " + isRunning);
        if (!isRunning) {
            return;
        }
        ...
        if (SimProcessorUtils.hasBasicPermissions(context)) {
            if (MtkTelephonyIntents.ACTION_PHB_STATE_CHANGED.equals(action)) {
                if (!userManager.isUserUnlocked()) {
                    ...
                    Log.e(TAG, "[onReceive] user locked, handle it later.");
                    return;
                }
        ...
        } else if (MtkTelephonyIntents.ACTION_PHB_STATE_CHANGED.equals(action)
