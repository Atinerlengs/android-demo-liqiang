...
//*/ freeme.zhaozehong, 20180123. for freemeOS, UI redesign
...
public class PhoneFavoritesTileAdapter extends BaseAdapter implements OnDragDropListener {
  ...
  private void saveCursorToCache(Cursor cursor) {
    ...
    /*/ freeme.zhaozehong, 20180327. for freemeOS, remove shortcut(long press dialer icon in launcher)
  ...
  public View getView(int position, View convertView, ViewGroup parent) {
    ...
    return tileView;
    /*/
    return getFreemeItemView(position, convertView, parent);
    //*/
  }
  ...
  private static class ViewTypes {
    ...
    //*/ freeme.zhaozehong, 20180123. for freemeOS, UI redesign
    private View getFreemeItemView(int position, View convertView, ViewGroup parent) {
        final ViewHolder holder;
        if (convertView == null) {
            convertView = View.inflate(mContext, R.layout.freeme_phone_favorite_tile_view, null);
            holder = new ViewHolder();
            holder.check = convertView.findViewById(R.id.contact_detail_icon);
            holder.name = convertView.findViewById(R.id.contact_tile_name);
            holder.tileTitleLayout = convertView.findViewById(R.id.tile_title_layout);
            holder.tileTitle = convertView.findViewById(R.id.tile_title);
            holder.speedItem = convertView.findViewById(R.id.speed_item);
            holder.bottomLine = convertView.findViewById(R.id.bottom_line);
            // hide title
            holder.tileTitleLayout.setVisibility(View.GONE);
        ...
        if (entry != null) {
            /*/ hide title
            if (mNumStarred > 0 && position < mNumStarred) {
                holder.tileTitleLayout.setVisibility(position == 0 ? View.VISIBLE : View.GONE);
                holder.tileTitle.setText(R.string.contactsFavoritesLabel);
            } else {
                holder.tileTitleLayout.setVisibility(position == mNumStarred
                        ? View.VISIBLE : View.GONE);
                holder.tileTitle.setText(R.string.favoritesFrequentContacted);
            }
            /*/
            holder.bottomLine.setVisibility(
                    mNumStarred > 0 && position == (mNumStarred - 1) && mNumFrequents > 0
                            ? View.GONE : View.VISIBLE);
            holder.speedItem.setVisibility(View.VISIBLE);
            convertView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View arg0) {
                    ...
                    CallSpecificAppData.Builder callSpecificAppData = CallSpecificAppData.newBuilder()
                    ...
                    if (entry.pinned != ContactsContract.PinnedPositions.UNPINNED) {
                    ...
                    if (TextUtils.isEmpty(entry.phoneNumber)) {
                        ...
                        mListener.onContactSelected(entry.lookupUri,
                                MoreContactUtils.getTargetRectFromView(holder.speedItem),
                        ...
                        mListener.onCallNumberDirectly(entry.phoneNumber, callSpecificAppData.build());
                    }
                }
            });
            ...
            if (entry == ContactEntry.BLANK_ENTRY) {
                ...
                //holder.check.setVisibility(entry.isFavorite ? View.VISIBLE : View.GONE);
                convertView.setVisibility(View.VISIBLE);
                holder.check.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        // Now perform the real save
                        FreemeIntentUtils.startQuickContact(mContext, entry.lookupUri,
                                mContext.getResources().getString(R.string.contactsFavoritesLabel));
            ...
            holder.tileTitle.setVisibility(View.GONE);
            holder.speedItem.setVisibility(View.GONE);
    ...
    private static class ViewHolder {
        TextView name;
        ImageView check;
        LinearLayout tileTitleLayout;
        TextView tileTitle;
        LinearLayout speedItem;
