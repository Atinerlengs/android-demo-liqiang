...
//*/ freeme.zhaozehong, 20180119. for freemeOS, UI redesign
import com.freeme.dialer.app.calllog.FreemeCallLogFragment;
import com.freeme.dialer.app.calllog.FreemeVisualVoicemailCallLogFragment;
...
public class DialtactsPagerAdapter extends FragmentPagerAdapter {
  ...
  /*/ freeme.zhaozehong, 20180124. for freemeOS, UI redesign
  ...
  //*/ freeme.zhaozehong, 20180116. for freemeOS, UI redesign
  private final boolean mUseFreemeUI = true;
  private FreemeContactsFragment mFreemeContactsFragment;
  private FreemeCallLogFragment mFreemeCallLogFragment;
  private FreemeCallLogFragment mFreemeVoicemailFragment;
  //*/
  ...
  public Fragment getItem(int position) {
    ...
    switch (getRtlPosition(position)) {
      ...
      case TAB_INDEX_HISTORY:
        //*/ freeme.zhaozehong, 20180116. for freemeOS, UI redesign
        if (mUseFreemeUI) {
            if (mFreemeCallLogFragment == null) {
                mFreemeCallLogFragment = new FreemeCallLogFragment(CallLogQueryHandler.CALL_TYPE_ALL);
            }
            return mFreemeCallLogFragment;
        }
        //*/
        if (useNewCallLogTab) {
      ...
      case TAB_INDEX_ALL_CONTACTS:
        //*/ freeme.zhaozehong, 20180116. for freemeOS, UI redesign
        if (mUseFreemeUI) {
            if (mFreemeContactsFragment == null) {
                mFreemeContactsFragment = new FreemeContactsFragment();
            }
            return mFreemeContactsFragment;
        }
        //*/
      ...
      case TAB_INDEX_VOICEMAIL:
        //*/ freeme.zhaozehong, 20180116. for freemeOS, UI redesign
        if (mUseFreemeUI) {
            if (mFreemeVoicemailFragment == null) {
                mFreemeVoicemailFragment = new FreemeVisualVoicemailCallLogFragment();
            }
            return mFreemeVoicemailFragment;
        }
        //*/
  ...
  public Fragment instantiateItem(ViewGroup container, int position) {
    ...
    //*/ freeme.zhaozehong, 20180116. for freemeOS, UI redesign
    else if (fragment instanceof FreemeContactsFragment) {
        mFreemeContactsFragment = (FreemeContactsFragment) fragment;
    } else if (fragment instanceof FreemeCallLogFragment) {
        mFreemeCallLogFragment = (FreemeCallLogFragment) fragment;
    } else if (fragment instanceof FreemeCallLogFragment && position == TAB_INDEX_VOICEMAIL) {
        mFreemeVoicemailFragment = (FreemeCallLogFragment) fragment;
        LogUtil.v("ViewPagerAdapter.instantiateItem", mFreemeVoicemailFragment.toString());
    }
    //*/
  ...
  public void removeVoicemailFragment(FragmentManager manager) {
    ...
    if (mFreemeVoicemailFragment != null) {
        manager.beginTransaction().remove(mFreemeVoicemailFragment).commitAllowingStateLoss();
        mFreemeVoicemailFragment = null;
    }
    //*/
    if (voicemailFragment != null) {
  ...
  public void setHasActiveVoicemailProvider(boolean hasActiveVoicemailProvider) {
    ...
    public void onAllSelected(int pageIdx) {
        if (pageIdx == DialtactsPagerAdapter.TAB_INDEX_HISTORY) {
            mFreemeCallLogFragment.onAllSelected();
        } else if (pageIdx == DialtactsPagerAdapter.TAB_INDEX_ALL_CONTACTS) {
            mFreemeContactsFragment.onAllSelected();
        }
    }

    public void onDialpadUp() {
        if (mFreemeCallLogFragment != null && mFreemeCallLogFragment.isVisible()) {
            mFreemeCallLogFragment.moveEmptyViewViaAnimation(true);
        }
    }

    public void onDialpadDown() {
        if (mFreemeCallLogFragment != null && mFreemeCallLogFragment.isVisible()) {
            mFreemeCallLogFragment.moveEmptyViewViaAnimation(false);
        }
    }
    //*/

    //*/ freeme.zhaozehong, 20180524. for freemeOS, UI redesign
    public boolean inSearchContactorMode() {
        return mFreemeContactsFragment != null && mFreemeContactsFragment.inSearchContactorMode();
    }

    public void clearSearchContactorFocus() {
        if (mFreemeContactsFragment != null){
            mFreemeContactsFragment.clearSearchContactorFocus();
        }
    }
    //*/

    //*/ freeme.liqiang, 20180809. refresh calllog item when delete contacts
    public void updateCalllogInfo() {
        if (mFreemeCallLogFragment != null) {
            mFreemeCallLogFragment.updateCalllogInfo();
        }
    }
    //*/
}
