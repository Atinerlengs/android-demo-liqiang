...
public final class CallLogListItemViewHolder extends RecyclerView.ViewHolder
  ...
  private CallLogListItemViewHolder(
    ...
    /*/ freeme.zhaozehong, 20180206. for freemeOS, UI redesign
    primaryActionView.setOnClickListener(mExpandCollapseListener);
    /*/
    callLogEntryView.setOnClickListener(mExpandCollapseListener);
    //*/
    if (mVoicemailPlaybackPresenter != null
      ...
      /*/
      callLogEntryView.setOnLongClickListener(longPressListener);
      //*/
      ...
      /*/ freeme.zhaozehong, 20180206. for freemeOS, replace ContextMenu with AlertDialog
  ...
  public static CallLogListItemViewHolder createForTest(
    ...
    /*/
    viewHolder.checkBoxView = new CheckBox(context);
  ...
  public boolean onMenuItemClick(MenuItem item) {
    ...
    if (resId == R.id.context_menu_copy_to_clipboard) {
      ...
      /*/
      intent.setClass(mContext, FreemeDialtactsActivity.class);
      //*/
      ...
      switch (resId) {
          ...
          case R.id.freeme_menu_item_call:
              call();
              return true;
          case R.id.freeme_menu_item_video_call:
          ...
          */
          ...
          case R.id.freeme_menu_item_add_new_contact:
              DialerUtils.startActivityWithErrorToast(mContext,
                      IntentUtil.getNewContactIntent(number));
              return true;
          case R.id.freeme_menu_item_add_exist_contact:
              DialerUtils.startActivityWithErrorToast(mContext,
                      IntentUtil.getAddToExistingContactIntent(number),
                      R.string.add_contact_not_available);
              return true;
          case R.id.freeme_menu_item_mark:
              return true;
          case R.id.freeme_menu_item_delete_calllog:
              deleteCallLogs();
              return true;
          case R.id.freeme_menu_item_mulit_delete_calllog:
              if (isMultiSelected) {
                  return true;
              }
              isMultiSelected = true;
              if (mMultiSelectCallBack != null) {
                  mMultiSelectCallBack.isInMulitMode(isMultiSelected);
              }
              return true;
      }
      //*/
  ...
  public void onClick(View view) {
    ...
    if (view.getId() == R.id.primary_action_button) {
      ...
      boolean isStranger = TextUtils.isEmpty(info.lookupKey);
      if (isStranger) {
          FreemeIntentUtils.startStrangerActivityWithErrorToast(mContext, number,
                  callTypeOrLocation, mCallTypeFilter,
                  mCallLogCache.isVoicemailNumber(accountHandle, number));
      } else {
          if (CompatUtils.hasPrioritizedMimeType()) {
              FreemeIntentUtils.showContactDetailsFromCallloglist(mContext,
                      info.lookupUri, true, number, mCallTypeFilter);
          } else {
              ContactsContract.QuickContact.showQuickContact(mContext, view, info.lookupUri,
  ...
  public void onCreateContextMenu(
    ...
    if (callType == CallLog.Calls.VOICEMAIL_TYPE) {
      ...
      menu.setHeaderTitle(PhoneNumberUtilsCompat.createTtsSpannable(
              BidiFormatter.getInstance().unicodeWrap(nameOrNumber, TextDirectionHeuristics.LTR)));
      //*/
    }
    ...
    if (PhoneNumberHelper.canPlaceCallsTo(number, numberPresentation)
      ...
      if (CallUtil.isVideoEnabled(mContext)) {
        menu.add(ContextMenu.NONE, R.id.freeme_menu_item_video_call, ContextMenu.NONE,
                R.string.search_shortcut_make_video_call).setOnMenuItemClickListener(this);
      }
      */
      menu.add(ContextMenu.NONE, R.id.freeme_menu_item_send_sms, ContextMenu.NONE,
              R.string.call_log_action_send_message).setOnMenuItemClickListener(this);
    }
    ...
    //*/ freeme.zhaozehong, 20180222. for freemeOS, UI redesign
    /*
    menu.add(ContextMenu.NONE, R.id.freeme_menu_item_mark, ContextMenu.NONE,
            R.string.freeme_stranger_mark_number).setOnMenuItemClickListener(this);
    */
    menu.add(ContextMenu.NONE, R.id.freeme_menu_item_delete_calllog, ContextMenu.NONE,
            R.string.delete_from_call_log_list).setOnMenuItemClickListener(this);
    menu.add(ContextMenu.NONE, R.id.freeme_menu_item_mulit_delete_calllog, ContextMenu.NONE,
            R.string.freeme_call_logs_mulit_delete).setOnMenuItemClickListener(this);
    //*/
  ...
  public interface OnClickListener {
    ...
    public static CallLogListItemViewHolder newCreate(
            View view,
            Context context,
            OnClickListener blockReportListener,
            View.OnClickListener expandCollapseListener,
        ...
        PhoneCallDetailsViews phoneViews = PhoneCallDetailsViews.fromView(view);
        phoneViews.initExtraViews(view);

        CallLogListItemViewHolder holder = new CallLogListItemViewHolder(
                context,
                blockReportListener,
                expandCollapseListener,
                longClickListener,
        ...
        holder.initExtraViews(view);

        return holder;
    }

    public View mActionButtonLayout;
    public View mCallDetail;
    private ViewStub mCheckBoxViewStub;
    private CheckBox checkBoxView;
    public boolean isMultiSelected;
    private IFreemeMultiSelectCallBack mMultiSelectCallBack;

    public void setMultiSelectCallBack(IFreemeMultiSelectCallBack mMultiSelectCallBack) {
        this.mMultiSelectCallBack = mMultiSelectCallBack;
    }

    private void initExtraViews(View view) {
        mActionButtonLayout = view.findViewById(R.id.primary_action_button);
        mActionButtonLayout.setOnClickListener(this);
        mCallDetail = view.findViewById(R.id.call_log_detail);
        mCheckBoxViewStub = view.findViewById(R.id.checkbox_container);
    }

    public void setCheckBoxViewVisiable(int visibility) {
        if (visibility == View.VISIBLE) {
            if (checkBoxView == null && mCheckBoxViewStub != null) {
                checkBoxView = mCheckBoxViewStub.inflate().findViewById(R.id.checkbox);
                checkBoxView.setOnClickListener((View v) -> {
                    if (mExpandCollapseListener != null && rootView != null) {
                        mExpandCollapseListener.onClick(rootView.findViewById(R.id.call_log_row));
                    }
                });
            }
            checkBoxView.setVisibility(visibility);
            mCallDetail.setVisibility(View.GONE);
        } else if (checkBoxView != null) {
            checkBoxView.setVisibility(visibility);
            mCallDetail.setVisibility(View.VISIBLE);
        }
    }

    public void updateCheckBoxCheckStatus(boolean isChecked) {
    ...
    public void call() {
        if (PhoneNumberHelper.canPlaceCallsTo(number, numberPresentation)) {
            ...
            if (!isVoicemailNumber && showLightbringerPrimaryButton()) {
                CallIntentBuilder.increaseLightbringerCallButtonAppearInCollapsedCallLogItemCount();
                provider = IntentProvider.getLightbringerIntentProvider(number);
            } else if (isVoicemailNumber) {
                provider = IntentProvider.getReturnVoicemailCallIntentProvider();
            } else if (confCallNumbers != null && DialerVolteUtils.isVolteConfCallEnable(mContext)) {
    ...
    private void deleteCallLogs() {
        ...
        CallLogAsyncTaskUtil.deleteCalls(mContext, callIdsBuffer.toString(),
                CallLogAsyncTaskUtil.DeleteType.CALL_IDS);
    }
    ...
    private Uri[] getCallLogEntryUris() {
        ...
        for (int index = 0; index < numIds; ++index) {
            uris[index] = ContentUris.withAppendedId(TelecomUtil.getCallLogUri(mContext), callIds[index]);
        }
        return uris;
    }
