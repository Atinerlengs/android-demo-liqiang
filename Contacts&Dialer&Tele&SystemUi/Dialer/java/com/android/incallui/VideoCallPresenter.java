...
public class VideoCallPresenter
   ...
   */
  private static final boolean MTK_CMCC_VILTE_LAB_TEST =
          "1".equals(SystemProperties.get("persist.radio.vilte.lab_op_code"));
  private static final boolean MTK_CT_VILTE_LAB_TEST =
          "9".equals(SystemProperties.get("persist.radio.vilte.lab_op_code"));
  /**
  ...
  public void onVideoCallScreenUiUnready() {
    ...
    if (mPrimaryCall != null) {
      ...
      /// call's quality test. @{
      mPrimaryCall.setLargePreview(false);
      /// @}
    }
 ...
 private void checkForVideoRingtoneStateChange(@Nullable DialerCall call) {
  ...
  private void enableCamera(VideoCall videoCall, boolean isCameraRequired) {
    ...
    final DialerCall activeCall = CallList.getInstance().getActiveCall();
    if (((mPrimaryCall != null) && (mPrimaryCall.getVideoPauseState() == true))
  ...
  private class LocalDelegate implements VideoSurfaceDelegate {
    ...
    public void onSurfaceClick(VideoSurfaceTexture videoCallSurface) {
      ...
      if (MTK_CMCC_VILTE_LAB_TEST || MTK_CT_VILTE_LAB_TEST) {
        LogUtil.i("VideoCallPresenter.onSurfaceClick", "in cmcc or ct lab test" );
        if (mPrimaryCall.getState() != DialerCall.State.ACTIVE) {
          return;
        }

        boolean oldstate = mPrimaryCall.isLargePreview();
        mPrimaryCall.setLargePreview(!oldstate);
        mVideoCallScreen.switchLocalAndPeer(!oldstate);
      } else {
      /// @}
  ...
  public boolean isPreviewLarge() {
    boolean previewLarge = mPrimaryCall!= null && mPrimaryCall.isLargePreview();
  ...
  private void updatePreviewHideState(DialerCall call) {
    ...
    mVideoCallScreen.hidePreview(isVideoCall(call)
        ...
        ///@}
    LogUtil.d("VideoCallPresenter.updatePreviewHideState",
  ...
  private boolean isScreenInForground() {
    ...
    InCallActivity incallactivity =  (InCallActivity) mVideoCallScreen.
