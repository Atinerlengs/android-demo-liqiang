...
//*/ freeme.zhaozehong, 20180419. for get sub info
...
public class BottomRow {
  ...
  public static class Info {
    ...
    //*/ freeme.zhaozehong, 20180228. for freemeOS, UI redesign
    public Info setFreemeExtras(boolean isShowSim, int simSlot) {
      mIsShowSim = isShowSim;
      mSimIconRes = getSimSlotIconRes(simSlot);
      return this;
    }

    private final int[] mSlotIconArr = new int[]{
            R.drawable.freeme_incallui_sim1_icon,
            R.drawable.freeme_incallui_sim2_icon,
    };

    private int getSimSlotIconRes(int slot) {
      if (slot >= 0 && slot < mSlotIconArr.length) {
        return mSlotIconArr[slot];
      } else {
        return 0;
      }
    }
  ...
  public static Info getInfo(Context context, PrimaryCallState state, PrimaryInfo primaryInfo) {
    ...
    /*/
    int slot = SubscriptionManager.INVALID_SIM_SLOT_INDEX;
    SubscriptionInfo subInfo = SubscriptionManager.from(context)
            .getActiveSubscriptionInfo(state.mSubId);
    if (subInfo != null) {
    ...
    return new Info(
            ...
            isSpamIconVisible,
            shouldPopulateAccessibilityEvent)
            .setFreemeExtras(primaryInfo.mIsShowSim, slot);
    //*/
  }
