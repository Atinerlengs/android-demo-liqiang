...
//*/
//*/ freeme.zhaozehong, 20180416. for freemeOS, show floating window
import com.freeme.incallui.floating.FreemeInCallFloatingService;
//*/
//*/ freeme.zhangyalun, 20180418. freeme caller suspension notification
...
//*/
//*/ freeme.zhaozehong, 20180929. restricted heads-up notification display
import com.freeme.incallui.utils.FreemeInCallFeatureOptions;
//*/
...
public class StatusBarNotifier
  ...
  public StatusBarNotifier(@NonNull Context context, @NonNull ContactInfoCache contactInfoCache) {
    ...
    //*/ freeme.zhaozehong, 20180929. restricted heads-up notification display
    mOptions = new FreemeInCallFeatureOptions(context);
    //*/
  }
  ...
  private void updateInCallNotification(CallList callList) {
    ...
    if (call != null) {
      ...
      //*/
      //*/ freeme.zhaozehong, 20180416. for freemeOS, show floating window
      FreemeInCallFloatingService.stop(mContext);
      //*/
    }
  ...
  private void buildAndSendNotification(
    ...
    switch (notificationType) {
      case NOTIFICATION_INCOMING_CALL:
        //*/ freeme.zhaozehong, 20180731. restricted heads-up notification display
        boolean isHeadsUp = !mOptions.forceFullScreenForIncomingCall();
        //*/
        if (BuildCompat.isAtLeastO()) {
          //*/ freeme.zhaozehong, 20180731. restricted heads-up notification display
          int newImportance = isHeadsUp ? NotificationManager.IMPORTANCE_MAX
                  : NotificationManager.IMPORTANCE_DEFAULT;
          int nowImportance = mNotificationManager
                  .getNotificationChannel(NotificationChannelId.INCOMING_CALL)
                  .getImportance();
          if (newImportance != nowImportance) {
            mNotificationManager.getNotificationChannel(NotificationChannelId.INCOMING_CALL)
    ...
    RemoteViews incomingCallRemoteViews = new RemoteViews(mContext.getPackageName(),
            R.layout.freeme_incoming_call_notification);
    incomingCallRemoteViews.setTextViewText(R.id.freeme_notification_caller, name);
    if (TextUtils.isEmpty(contactInfo.namePrimary)) {
      incomingCallRemoteViews.setTextViewText(R.id.freeme_notification_incoming_summary,
              contactInfo.location);
    } else {
      incomingCallRemoteViews.setTextViewText(R.id.freeme_notification_incoming_summary,
              contactInfo.number+" "+contactInfo.location);
    }
    incomingCallRemoteViews.setImageViewResource(R.id.freeme_notification_sim_indicate, simicon);
    ...
    //*/ freeme.zhangyalun, 20180418. freeme caller suspension notification
    bindAction(incomingCallRemoteViews);
    builder.setCustomHeadsUpContentView(incomingCallRemoteViews);
    //*/
  ...
  private boolean checkForChangeAndSaveData(
     ...
     * M: ALPS03836283, Increase call performace. Move here to record the variables to skip
  ...
  private void addAnswerAction(Notification.Builder builder) {
    ...
    mFreemeActions.add(new FreemeCallAction(
            R.drawable.freeme_incoming_call_notification_answer_audio_selector,
            answerVoicePendingIntent).setActionType(FreemeCallAction.TYPE_ACTION_ANSWER_AUDIO));
    //*/
  }
  ...
  private void addDismissAction(Notification.Builder builder) {
    ...
    //*/ freeme.zhangyalun, 20180418. freeme caller suspension notification
    mFreemeActions.add(new FreemeCallAction(
            R.drawable.freeme_incoming_call_notification_reject_selector,
            declinePendingIntent).setActionType(FreemeCallAction.TYPE_ACTION_REJECT));
    //*/
  }
  ...
  private void addHangupAction(Notification.Builder builder) {
    ...
    mFreemeActions.add(new FreemeCallAction(
            R.drawable.freeme_incoming_call_notification_reject_selector,
            hangupPendingIntent).setActionType(FreemeCallAction.TYPE_ACTION_REJECT));
    //*/
  }
  ...
  private void addVideoCallAction(Notification.Builder builder) {
    ...
    //*/ freeme.zhangyalun, 20180418. freeme caller suspension notification
    mFreemeActions.add(new FreemeCallAction(
            R.drawable.freeme_incoming_call_notification_answer_video_selector,
            answerVideoPendingIntent).setActionType(FreemeCallAction.TYPE_ACTION_ANSWER_VIDEO));
    //*/
  }
  ...
  private void addAcceptUpgradeRequestAction(Notification.Builder builder) {
    ...
    mFreemeActions.add(new FreemeCallAction(
            R.drawable.freeme_incoming_call_notification_answer_video_selector,
            acceptVideoPendingIntent).setActionType(FreemeCallAction.TYPE_ACTION_ANSWER_VIDEO));
    //*/
  }
  ...
  private void addDismissUpgradeRequestAction(Notification.Builder builder) {
    ...
    mFreemeActions.add(new FreemeCallAction(
            R.drawable.freeme_incoming_call_notification_reject_selector,
            declineVideoPendingIntent).setActionType(FreemeCallAction.TYPE_ACTION_REJECT));
    //*/
  }
  ...
  private Notification.Builder getNotificationBuilder() {
    ...
    //*/ freeme.zhaozehong, 20180731. restricted heads-up notification display
    if (!mOptions.forceFullScreenForIncomingCall())
    //*/
  ...
  public void onIncomingCall(InCallState oldState, InCallState newState,
    ...
    private void bindAction(RemoteViews view) {
        view.removeAllViews(R.id.action_layout);
        for (FreemeCallAction action : mFreemeActions) {
            RemoteViews actionView = new RemoteViews(mContext.getPackageName(),
                    R.layout.freeme_incoming_call_notification_action);
            int view_id = R.id.freeme_notification_action;
            actionView.setOnClickPendingIntent(view_id, action.getPendingIntent());
            actionView.setImageViewResource(view_id, action.getIcon());
            view.addView(R.id.action_layout, actionView);
        }
    }
    //*/

    //*/ freeme.zhaozehong, 20180929. restricted heads-up notification display
    private FreemeInCallFeatureOptions mOptions;
    //*/
}
