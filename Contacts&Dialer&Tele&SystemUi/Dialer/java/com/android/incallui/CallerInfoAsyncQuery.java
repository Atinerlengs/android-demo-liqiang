        ...
        if (mCallerInfo == null) {
          ...
          if (cw.event == EVENT_EMERGENCY_NUMBER) {
            ...
            if (!TextUtils.isEmpty(cw.number)) {
              mCallerInfo.phoneNumber = cw.number;
              //*/ freeme.zhaozehong, 20180314. for freemeOS, UI redesign
              if (!mCallerInfo.contactExists) {
                FreemeYellowPageUtils.YellowPageData yData =
                        new FreemeYellowPageUtils().getFreemeYellowPagePublicInfo(
                                GlobalEnv.getApplicationContext(), cw.number);
                if (yData != null) {
                  mCallerInfo.name = yData.getName();
                  mCallerInfo.isCachedPhotoCurrent = true;
                  mCallerInfo.cachedPhotoIcon = FreemeBitmapUtils
                          .getRoundedCornerBitmapFromByte(yData.getIcon());
                }
              }
              //*/
            }
