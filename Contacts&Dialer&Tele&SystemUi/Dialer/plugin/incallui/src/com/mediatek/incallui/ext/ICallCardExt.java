package com.mediatek.incallui.ext;

import android.content.Context;
import android.database.Cursor;
import android.graphics.Bitmap;
import android.graphics.drawable.Drawable;
import android.telecom.PhoneAccount;
import android.telecom.PhoneAccountHandle;
import android.view.View;

public interface ICallCardExt {
    /**
     * called when CallCard view created, based on CallCardFragment lifecycle
     *
     * @param context  host context
     * @param rootView the CallCardFragment view
     */
    void onViewCreated(Context context, View rootView);

    /**
     * called when CallCard view destroy, based on CallCardFragment lifecycle
     *
     */
    public void onDestroyView();

    /**
     * called when secondary info view clicked
     *
     * @param clist  host CallList
     * @return boolean should skip host click logic
     */
    public boolean handleOnClick(Object clist);

    /**
     * called when onStateChange of CallCardPresenter
     *
     * @param clist  host CallList
     */
    public void onIncomingStateChange(Object clist);

    /**
     * Check if need set Secondary call info
     *
     * @return boolean should skip host update
     */
    public boolean needUpdateSecondaryCall();

    /**
     * called when call state changed, based on onStateChange
     *
     * @param call the call who was changed
     */
    void onStateChange(android.telecom.Call call);

    /**
     * called when there is an request to update the Primary call card
     * information UI. such as: call state changed, contact info retrieved, ui
     * ready
     *
     * @param call the primary call, Notice, call would be null
     */
    void updatePrimaryDisplayInfo(android.telecom.Call call);

    /**
     * Return the icon drawable to represent the call provider.
     *
     * @param context for get service.
     * @param handle for get icon.
     * @return The icon.
     */
    Drawable getCallProviderIcon(Context context, PhoneAccountHandle handle);

    /**
     * Return the string label to represent the call provider.
     *
     * @param context for get service.
     * @param account for get lable.
     * @return The lable.
     */
    String getCallProviderLabel(Context context, PhoneAccountHandle handle);

    /**
     * set phone account for call.
     *
     * @param account call via account.
     * @internal
     */
    void setPhoneAccountForSecondCall(PhoneAccount account);

    /**
     * set phone account for call.
     *
     * @param account call via account.
     * @internal
     */
    void setPhoneAccountForThirdCall(PhoneAccount account);

    /**
     * Called when op09 plug in need to show call account icon.
     *
     * @return true if need to show.
     */
    boolean shouldShowCallAccountIcon();

    /**
     * To get phone icon bitmap object of some call.
     *
     * @return bitmap
     */
    Bitmap getSecondCallPhoneAccountBitmap();

    /**
     * To get phone icon bitmap object of some call.
     *
     * @return bitmap
     */
    Bitmap getThirdCallPhoneAccountBitmap();

    /**
     * To get provider label for call.
     *
     * @return provider label
     */
    String getSecondCallProviderLabel();

    /**
     * To get provider label for call.
     *
     * @return provider label
     */
    String getThirdCallProviderLabel();

    /**
     * Customize the CallerInfo after it is generated after query.
     * Plugin is able to modify any field of the CallerInfo if necessary.
     * Plugin can decide the content shown in the Call Card.
     *
     * @param context    the InCallUI context.
     * @param callerInfo the CallerInfo generated.
     * @param cursor     the cursor contains the caller info raw data.
     * @internal
     */
    void customizeCallerInfo(Context context, Object callerInfo, Cursor cursor);

    /**
     * Called when incallui create hold framgent.
     * Using for update secondary sim.
     *
     * @param view    the hold fragment view.
     */
    void onHoldViewCreated(View view);

    /**
     * Called when state changed in callcard presenter.
     * Using for update secondary sim.
     *
     * @param primaryCall       the primary call in callcard presenter.
     * @param secondaryCall    the secondary call in callcard presenter.
     */
    void onStateChange(Object primaryCall, Object secondaryCall);

}
