...
public class FreemeActionBarWithoutSplitMenu extends Activity {
...
//        getWindow().setUiOptions(ActivityInfo.UIOPTION_SPLIT_ACTION_BAR_WHEN_NARROW);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        addOptionsMenu(menu, "ACTIONMODE");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == android.R.id.home) {
            ...
            setBackTitleColor();
            setUpViewColor();
        } else if (item.getTitle().equals("ACTIONMODE")) {
            startActionMode();
        } else {
    ...
    private class MyActionModeCallback implements ActionMode.Callback {
        @Override public boolean onCreateActionMode(ActionMode mode, Menu menu) {
            ...
            MenuItem actionItem1 = menu.add("DIS").setEnabled(false);
            actionItem1.setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS);

            // action mode default mode is SHOW_AS_ACTION_IF_ROOM.
            menu.add("STOP").setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS);
            menu.add("STOP").setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS);
            menu.add("STOP");
            menu.add("STOP");
            menu.add("STOP");
