...
/**
 * Class to show Network plmn as Prefernce for edit.
 */
public class PLMNListPreference extends TimeConsumingPreferenceActivity
    ...
    /**
     * Handling for preference tree click.
     * @param preferenceScreen Network plmn preference screen.
     * @param pref Network plmn preference.
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        menu.add(0, MENU_ADD, 0, R.string.plmn_list_setting_add_plmn)
                /*/ freeme.liqiang, 20180404. adjust menu item
    ...
    private void initPlmnList(TimeConsumingPreferenceListener listener, boolean skipReading) {
        ...
        if (!skipReading) {
            ...
            gsmCdmaphone.getPol(mHandler.obtainMessage(
    ...
    /**
     * Query network selection event finished or not.
     * @param preference Network plmn preference.
     * @param reading Check reading done or not.
     */
    ...
    private void getSIMCapability() {
        MtkGsmCdmaPhone gsmCdmaphone =
                    (MtkGsmCdmaPhone) PhoneFactory.getPhone(mPhone.getPhoneId());
        gsmCdmaphone.getPolCapability(mHandler.obtainMessage(MyHandler.MESSAGE_GET_PLMN_CAPIBILITY,
                0, MyHandler.MESSAGE_GET_PLMN_CAPIBILITY));
    }
    ...
    private void refreshPreference(ArrayList<NetworkInfoWithAcT> list) {
        ...
        for (NetworkInfoWithAcT network : list) {
            if (addPLMNPreference(network, order) == true) {
                order++;
            }
    ...
    private boolean addPLMNPreference(NetworkInfoWithAcT network, int order) {
        int act = network.getAccessTechnology();
        /// for ALPS01659203 @{
        // when modem capability is not 4G, we should not show 4G PLMN.
        Log.d(LOG_TAG, "act: " + act);

        //M:03518516 Update handling for plmn list 4G preference
        if (!FeatureOption.isMtkLteSupport()) {
            if ((act & NetworkEditor.RIL_4G) != 0) {
                return false;
            }
        }
    ...
    protected void onActivityResult(final int requestCode,
        ...
        if (intent != null && TelephonyUtils.isSimStateReady(
                mPhone.getPhoneId()) && mPLMNList != null) {
        /// @}
    ...
    private void handleSetPLMN(ArrayList<NetworkInfoWithAcT> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            MtkGsmCdmaPhone gsmCdmaphone =
                    (MtkGsmCdmaPhone) PhoneFactory.getPhone(mPhone.getPhoneId());
            gsmCdmaphone.setPolEntry(ni, mHandler.obtainMessage(
    ...
    private void handlePLMNListDelete(NetworkInfoWithAcT network) {
        ...
        for (int i = list.size(); i < oldLength; i++) {
            NetworkInfoWithAcT ni = new NetworkInfoWithAcT("", "", 1, i);
            list.add(ni);
        }
        adjustPriority(list);
    ...
    /**
     * Class to show the SIM Network Capability.
     */
    ...
    private String getSubTitle(Intent intent) {
        ...
        if (TextUtils.isEmpty(title)) {
            title = getString(R.string.plmn_list_setting_title);
        }
