...
public class CdmaNetworkSettings {
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (TelephonyIntents.ACTION_SIM_STATE_CHANGED.equals(action)) {
                ...
                if (slotId == SubscriptionManager.getPhoneId(mPhone.getSubId())) {
                    updateSwitch();
                }
            }
        }
     ...
     * @param prefS the preference to be used.
    ...
    public CdmaNetworkSettings(PreferenceActivity activity, PreferenceScreen prefS, Phone phone) {
        ...
        if(SubscriptionManager.getDefaultDataSubscriptionId() == mPhone.getSubId()) {
            mActivity.getContentResolver().registerContentObserver(Settings.Global.getUriFor(
                    Settings.Global.MOBILE_DATA + mPhone.getSubId()),
                    true, mContentObserver);
        }
    ...
    private void handleEnable4GDataOnlyClick(Preference preference) {
        ...
        int settingsNetworkMode = android.provider.Settings.Global.getInt(
                mPhone.getContext().getContentResolver(),
                android.provider.Settings.Global.PREFERRED_NETWORK_MODE +
                mPhone.getSubId(), preferredNetworkMode);
        if (settingsNetworkMode != Phone.NT_MODE_GLOBAL) {
            settingsNetworkMode = Phone.NT_MODE_LTE_CDMA_EVDO_GSM_WCDMA;
        }
        int modemNetworkMode = isChecked ? MtkGsmCdmaPhone.NT_MODE_LTE_TDD_ONLY
                : settingsNetworkMode;
    ...
    private void updateEnableSingle4GDataNetworkUIFromDb() {
        ...
        boolean isChecked = settingsNetworkMode ==
                MtkGsmCdmaPhone.NT_MODE_LTE_TDD_ONLY ? true : false;
        mEnable4GDataPreference.setChecked(isChecked);
    }
