    ...
    */
    ...
    public static boolean isSvlteSlotInserted() {
        ...
        if (SubscriptionManager.isValidSubscriptionId(getCdmaSubId())) {
            int slotId = SubscriptionManager.getSlotIndex(getCdmaSubId());
    ...
    public static boolean getRadioStateForSlotId(final int slotId) {
        int currentSimMode = Settings.System.getInt(PhoneGlobals.getInstance()
                .getContentResolver(), MtkSettingsExt.Global.MSIM_MODE_SETTING, -1);
    ...
    public static boolean isCtVolteEnabled() {
        String volteValue = SystemProperties.get("persist.mtk_ct_volte_support");
        boolean result = (volteValue.equals("1") || volteValue.equals("2"));
        Log.d(TAG, "volteValue = " + volteValue + ", isCtVolteEnabled " + result);
        return result;
    }

    public static boolean isCtAutoVolteEnabled() {
        boolean result = SystemProperties.get("persist.mtk_ct_volte_support").equals("2");
        Log.d(TAG, "autoVolte = " + result);
        return result;
    }
    ...
    */
    public static boolean is4GDataOnly(Context context) {
        ...
        if (context != null && SubscriptionManager.isValidSubscriptionId(getCdmaSubId())) {
            ...
            if (networkMode == MtkGsmCdmaPhone.NT_MODE_LTE_TDD_ONLY) {
                result = true;
           }
        }
