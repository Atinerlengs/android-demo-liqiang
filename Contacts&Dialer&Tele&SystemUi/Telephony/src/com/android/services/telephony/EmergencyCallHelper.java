...
public class EmergencyCallHelper implements EmergencyCallStateListener.Callback {
    ...
    // mDialByNormal: Flag to show if this emergency call will be dialed by normal command (ATD)
    ...
    public void enableEmergencyCalling(EmergencyCallStateListener.Callback callback) {
        ...
        for (int i = 0; i < TelephonyManager.getDefault().getPhoneCount(); i++) {
            ...
            if (mDialByNormal && phone.getPhoneId() == mEccPhoneId) {
                mListeners.get(i).setWaitForInService(true);
                Log.d(this, "waitForInService set true (i=" + i
                        + ", mDialByNormal=" + mDialByNormal + ", phone.getPhoneId()="
                        + phone.getPhoneId() + ", mEccPhoneId=" + mEccPhoneId + ")");
            }
    ...
    public void setEccByNormalPhoneId(int phoneId) {
        mDialByNormal = true;
        mEccPhoneId = phoneId;
    }
