...
import com.mediatek.phone.ext.DigitsPhoneAccountExtBase;
...
public class TelecomAccountRegistry {
    ...
    public class AccountEntry implements PstnPhoneCapabilitiesNotifier.Listener {
        ...
        /// M: For [Digits] extension, the plugin sub class need use it. change to protected. @{
        ...
        public AccountEntry(Phone phone, boolean isEmergency, boolean isDummy) {
            mPhone = phone;
            ...
            /// M: For [Digits] plugin to replace PstnIncomingCallNotifier instance.
            mIncomingCallNotifier = makePstnIncomingCallNotifier(this, mPhone,
         ...
         */
        protected PhoneAccount registerPstnPhoneAccount(boolean isEmergency, boolean isDummyAccount) {
            ...
            /// M: [RTT] Need the CAPABILITY_RTT for RTT projects.
            capabilities = ExtensionManager.getRttUtilExt()
                    .combineRttPhoneAccountCapabilities(capabilities);
        ...
        /**
         * For [Digits] extension, the plugin sub class need use it. change to protected.
        ...
        // M: fix CR:ALPS02837867,selectAccount show order error about
        // sim2,sim1.add phoneId as new OrderKey about PhoneAccount.@{
        private Bundle putSortKey(Bundle extra) {
            Bundle newExtra = extra;
            if (newExtra == null) {
                newExtra = new Bundle();
            }
            newExtra.putInt(MtkPhoneAccount.EXTRA_PHONE_ACCOUNT_SORT_KEY, mPhone.getPhoneId());
            return newExtra;
        }
        // / @}
    }
    ...
    protected List<AccountEntry> mAccounts = new LinkedList<AccountEntry>();
    protected Object mAccountsLock = new Object();
    ///@}
    ...
    public static synchronized final TelecomAccountRegistry getInstance(Context context) {
        if (sInstance == null && context != null) {
            ...
            sInstance = (TelecomAccountRegistry) ExtensionManager.getDigitsUtilExt().
                    replaceTelecomAccountRegistry(sInstance, context);
        }
        return sInstance;
    }
    ...
    protected void setupAccounts() {
        ...
        synchronized (mAccountsLock) {
            if (phoneAccountsEnabled) {
                ...
                /// M: [Digits] Plugin would add virtual PhoneAccounts to mAccounts if necessary.
                addVirtualPhoneAccountEntries(mAccounts);
            }
    ...
    /**
     * M: For [Digits] extension, the plugin might call another constructor.
     */
