...
final class TtyManager {
                    ...
                    if (ar.exception != null) {
                        ...
                        if (ttyMode != mTtyMode) {
                            Log.d(TtyManager.this, "setting TTY mode failed, attempted %d, got: %d",
                                    mTtyMode, ttyMode);
                            ExtensionManager.getGttInfoExt().writeSettingsAndShowToast(ttyMode);
                        } else {
                            Log.d(TtyManager.this, "setting TTY mode to %d succeeded", ttyMode);
                            ExtensionManager.getGttInfoExt().updateAudioTtyMode(ttyMode);
                        }
                    }
                    break;
                }
    ...
    private static int telecomModeToPhoneMode(int telecomMode) {
        switch (telecomMode) {
            ...
            // and HCO to FULL
            /// M: AT command support 0~3 in MTK's design. @{
