...
//*/ freeme.zhaozehong, 20180129. for freemeOS, Accessibility Settings for incoming call
 ...
 */
public class CallNotifier extends Handler implements
    ...
    private void registerForNotifications() {
        //*/ freeme.zhaozehong, 17-1-6. register call state changed, used for reverse silent
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            //*/ freeme.zhaozehong, 17-1-6. for reverse silent
    ...
    private void onSuppServiceFailed(AsyncResult r) {
        ...
        // MTK-START: Fix the issue that dialog cannot be seen.
        /*
         * PhoneDisplayMessag will dismiss the previous dialog first then show the
         * new dialog, as a result, we need to remove any pending messages to dismiss
         * the dialog, otherwise, the new dialog may not be seen.
         */
    ...
    //*/ freeme.zhaozehong, 17-1-6. for reverse silent & pocket model
    public static final int PHONE_STATE_CHANGED = 1;
    private FreemeCallAccessibility mFreemeCallAccessibility;

    private void onPhoneStateChanged() {
        PhoneConstants.State state = mCM.getState();
        if (VDBG) log("onPhoneStateChanged: state = " + state);
        if (state == PhoneConstants.State.RINGING) {
            if (mFreemeCallAccessibility == null) {
                mFreemeCallAccessibility = new FreemeCallAccessibility(mApplication,
                        FreemeCallAccessibility.TYPE_REVERSE_MUTE
                                | FreemeCallAccessibility.TYPE_POCKET_MODE);
            }
            mFreemeCallAccessibility.start();
        } else if (state == PhoneConstants.State.OFFHOOK) {
            if (mFreemeCallAccessibility != null) {
                mFreemeCallAccessibility.stop();
            }
        } else if (state == PhoneConstants.State.IDLE) {
            if (mFreemeCallAccessibility != null) {
                mFreemeCallAccessibility.reset();
                mFreemeCallAccessibility = null;
            }
        }
    }
    //*/
