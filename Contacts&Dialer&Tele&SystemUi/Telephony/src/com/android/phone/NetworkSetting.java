...
public class NetworkSetting extends PreferenceActivity
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        if (preference == mSearchButton) {
            ...
            if (phone != null) {
                /// M: opeator sepc, when insert CU card, on CU load.
                /// seatch is not CU network ,
                /// add forbidden and click show toast, can not regsiter. @{
                if (!ExtensionManager.getNetworkSettingExt()
                        .onPreferenceTreeClick(mNetworkMap.get(selectedCarrier), mPhoneId)) {
                    /// }@
    ...
    private String getSubTitle(Intent intent) {
        ...
        if (TextUtils.isEmpty(title)) {
            title = getString(R.string.networks);
        }
