...
public class PhoneGlobals extends ContextWrapper {
    ...
    private static final String SIM_SERVICE_ACTION = "com.mediatek.settings.sim.SIM_SELECT_SERVICE";
    ...
    private void maybeTurnCellOff(Context context, boolean isAirplaneNewlyOn) {
        if (isAirplaneNewlyOn) {
            ...
            if (mTelEx != null && mTelEx.isEccInProgress()) {
            /// @}
    ...
    private void updateDataRoamingStatus() {
        ...
        if (!mNoDataDueToRoaming
                ...
                ExtensionManager.getSsRoamingServiceExt().isDisplayNotificationForRoaming(this))
                && ExtensionManager.getSsRoamingServiceExt().
                isNotificationForRoamingAllowed(this)) {
    ...
    private void registerSubInfoRecordIntent() {
        Log.d(LOG_TAG, "registerSubInfoRecordIntent");
        try {
            Thread simEventThread = new Thread() {
                public void run() {
                    Intent intent = new Intent(SIM_SERVICE_ACTION);
                    intent.setPackage("com.android.settings");
                    Log.d(LOG_TAG, "Start Sim Select Service");
                    startService(intent);
                }
            };
            simEventThread.start();
        } catch (SecurityException e) {
            Log.e(LOG_TAG, "SimSelectService start failed");
        }
    }
}
