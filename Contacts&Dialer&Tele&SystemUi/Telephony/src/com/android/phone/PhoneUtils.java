...
public class PhoneUtils {
                        ...
                        public void onClick(DialogInterface dialog, int whichButton) {
                            switch (whichButton) {
                                case DialogInterface.BUTTON_POSITIVE:
                                    if (ExtensionManager.getMmiCodeExt()
                                                .showUssdInteractionDialog(phone, inputText)) {
                                        break;
                                    }
        ...
        if (userDataObject instanceof Uri) {
            ...
            if (cit.asyncQuery != null) {
                ...
                if (!TextUtils.isEmpty(updatedNumber)) {
                    ...
                    // For scenarios where we may receive a valid number from the network but a
                    // restricted/unavailable presentation, we do not want to perform a contact
                    // query (see note on isFinal above). So we set isFinal to true here as well.
                    if (cit.currentInfo.numberPresentation != PhoneConstants.PRESENTATION_ALLOWED) {
                        cit.isFinal = true;
                    } else {
                        cit.asyncQuery = CallerInfoAsyncQuery.startQuery(QUERY_TOKEN, context,
    ...
    static String getCompactNameFromCallerInfo(CallerInfo ci, Context context) {
        ...
        if ((compactName == null) || (TextUtils.isEmpty(compactName))) {
            ...
            } else if (ci != null && ci.numberPresentation == PhoneConstants
    ...
    public static PhoneAccountHandle makePstnPhoneAccountHandleWithPrefix(
        ...
        if (!isEmergency) {
            phoneIccId = phone.getFullIccSerialNumber();
            int slotId = SubscriptionController.getInstance().getSlotIndex(phone.getSubId());
            phoneIccId = !TextUtils.isEmpty(phoneIccId) ?
                    phoneIccId : TelephonyManager.getDefault().getSimSerialNumber(phone.getSubId());
            log("Non ECC PhoneAccountHandle mId=" + phoneIccId + ", slotId=" + slotId);
        }
        /// FIXME: when the phoneIccId == null, this would make String "null" assign to id.
        String id = isEmergency ? EMERGENCY_ACCOUNT_HANDLE_ID : prefix + String.valueOf(phoneIccId);
        /// @}
    ...
    public static Phone getPhoneForPhoneAccountHandle(PhoneAccountHandle handle) {
        if (handle != null && handle.getComponentName().equals(getPstnConnectionServiceName())) {
            /// M: [Digits] For plugin get iccId from phoneAccountHandle. @{
            //  Ensure always can get iccId even if it is virtualPhoneAccountHandle
            String iccId = ExtensionManager.getDigitsUtilExt()
                    .getIccidFromPhoneAccountHandle(handle);
            return getPhoneFromIccId(iccId);
            /// @}
        }
        return null;
    }
